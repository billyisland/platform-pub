...
Last worked: 2026-04-14. Completed: Universal Feed audit triage pass 1 — priority fixes from `universal-feed-audit.md` landed. Critical: C1 (explore-feed score ordering preserved, new-user cards interleaved at fixed positions), C2 (Jetstream listener leader-elected via `pg_try_advisory_lock`, self-scheduling DID refresh), C3 (RSS `fetch_interval_seconds` resets on recovery), C4 (Mastodon quote appends source URL, unknown action_types throw). Security: S2 (Nostr WS URLs SSRF-validated via new `validateWebSocketUrl`), S3 (future-timestamp Nostr events rejected before cursor advance), S5/S6 (subscribe input per-protocol validated; `relayUrls` capped at 10, scheme-checked), partial S1 (IP range list extended — IPv4-mapped IPv6, multicast v4/v6). Design: D1 (migration 060 — DB-backed `DbStateStore` for atproto OAuth authorize→callback, pruned every 5m), D2 (`clientPromise` cache clears on rejection). Remaining audit items: S1 DNS pinning, S4, K1–K6, D3–D6, minors.
Previous: 2026-04-14 earlier — Universal Feed Phase 4 — Mastodon ingestion (ActivityPub outbox polling + WebFinger resolution).
