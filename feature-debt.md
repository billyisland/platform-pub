...
**Active branch:** `workspace-experiment` (slices 1–32 complete + refactoring pass + hardening pass, browser testing in progress). Refactoring pass (2026-05-15): extracted shared `useResolverInput` hook + `resolve.ts` utilities from duplicated VesselBar/FeedComposer resolver logic; consolidated Zustand store setters via `patchVessel` helper; deleted dead `ForkFeedPrompt.tsx` + `ResetLayoutConfirm.tsx`; fixed duplicate `NewUserItem` type + `catch (err: any)`. Hardening pass (2026-05-24): sampling mode API consistency, `any` types removed from feed API layer, relay URL + avatar URL + tag name validation, partial index on `feed_sources` for active sources (migration 089). Full codebase review completed (REVIEW-PLAN.md, 24 sessions, 300+ diagnoses). Decision pending: merge or retire.
