...
export function NoteComposer({ onPublished }: NoteComposerProps) {
  ...
  if (!user) return null
  const charCount = content.length, isOver = charCount > NOTE_CHAR_LIMIT, isEmpty = content.trim().length === 0
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => { const input = document.createElement('input'); input.type = 'file'; input.accept = 'image/jpeg,image/png,image/gif,image/webp'; input.onchange = async (e) => { const file = (e.target as HTMLInputElement).files?.[0]; if (!file) return; setUploading(true); try { const r = await uploadImage(file); setContent(p => p + (p ? '\n' : '') + r.url) } catch (err) { setError(err instanceof Error ? err.message : 'Upload failed') } finally { setUploading(false) } }; input.click() }} disabled={uploading} className="btn-soft disabled:opacity-40 py-1.5 px-3 text-ui-xs">{uploading ? 'Uploading...' : 'Image'}</button>
              <button onClick={handlePost} disabled={!canPost} className="btn disabled:opacity-40 py-1.5 px-4 text-ui-xs">{publishing ? 'Posting...' : 'Post'}</button>
            </div>
          </div>
        </div>
      </div>
