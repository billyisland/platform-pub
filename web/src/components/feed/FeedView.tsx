...
type FeedBlock =
  ...
  | {
      ...
      leadsBriefRun: boolean;
    };

function isBrief(
  item: GlobalFeedItem,
): item is ArticleEvent & { type: "article" } {
...
export function FeedView() {
    ...
    async function loadFeed() {
      ...
      try {
        ...
        const feedOnlyIds = items
          .filter(
            (i): i is FeedItem | ExternalFeedItem => i.type !== "new_user",
          )
          .map((i) => i.id);
  ...
  const handleNoteDeleted = useCallback((id: string) => {
    setGlobalItems((prev) =>
      prev.filter((i) => i.type === "new_user" || i.id !== id),
    );
  }, []);
      ...
      <div className="pb-10">
        {globalLoading ? (
          ...
          <FeedErrorState
            ...
            showGatewayHint={showGatewayHint}
          />
          ...
          <div className="px-6 pt-[48px]">
            {layoutBlocks(globalItems).map((block, blockIdx) => {
              const marginTop =
                blockIdx === 0 ? 0 : block.leadsBriefRun ? 72 : 40;
              if (block.kind === "brief-pair") {
                ...
                return (
                  <div
                    key={`pair-${a.id}-${b.id}`}
                    style={{ marginTop }}
                    className="grid grid-cols-2 gap-x-[40px]"
                  >
                    <ArticleCard
                      article={a}
                      onQuote={handleQuote}
                      voteTally={voteTallies[a.id]}
                      myVoteCounts={myVoteCounts[a.id]}
                    ...
                    <ArticleCard
                      article={b}
                      onQuote={handleQuote}
                      voteTally={voteTallies[b.id]}
                      myVoteCounts={myVoteCounts[b.id]}
                ...
                <div key={item.id} style={{ marginTop }}>
                  {item.type === "article" ? (
                    <ArticleCard
                      article={item}
                      onQuote={handleQuote}
                      voteTally={voteTallies[item.id]}
                      myVoteCounts={myVoteCounts[item.id]}
...
function NewUserCard({ item }: { item: NewUserItem }) {
  ...
  return (
    <div className="flex items-center gap-3 py-3">
      ...
      <span className="text-ui-xs text-grey-600 flex-shrink-0">
...
function FeedErrorState({
  ...
  onRetry: () => void;
  showGatewayHint: boolean;
}) {
  return (
    <div className="flex items-start justify-center px-6 pt-[33vh] pb-20">
      <div className="text-center">
        <p className="text-[15px] text-black leading-[1.55]">
