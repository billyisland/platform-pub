...
export function ArticleComposePanel() {
  ...
  const [pricePence, setPricePence] = useState(
    user?.defaultArticlePricePence ?? 0,
  );
  ...
  const selectedPubIdRef = useRef(selectedPubId);
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-lg max-w-none focus:outline-none min-h-[200px] article-compose-body",
      },
    },
    onUpdate: ({ editor }) => {
      ...
      autoSaver(
        {
          ...
          publicationId: selectedPubIdRef.current,
        },
        (saved) => {
  ...
  // Seed editor body from note content when switching note → article
  useEffect(() => {
    if (!editor || !initialArticleContent || articleDraftId) return;
    editor.commands.setContent(initialArticleContent);
  }, [editor, initialArticleContent, articleDraftId]);
  ...
  // Register flushDraft with the store so the overlay can flush before closing
  useEffect(() => {
    setArticleFlush(() => flushDraft().then(() => {}));
    return () => setArticleFlush(null);
  }, [flushDraft, setArticleFlush]);
  ...
  const buildPublishData = useCallback((): PublishData | null => {
    ...
    if (isPaywalled) {
      ...
      if (idx !== -1) {
        ...
        paywallContent = fullContent
          .slice(idx + PAYWALL_GATE_MARKER.length)
          .trim();
        ...
        gatePositionPct =
          total > 0
            ? Math.min(
                99,
                Math.max(1, Math.round((freeContent.length / total) * 100)),
              )
            : 50;
      }
    }
  ...
  const canPublish =
    !publishing && !loadingDraft && title.trim().length > 0 && wordCount >= 10;
