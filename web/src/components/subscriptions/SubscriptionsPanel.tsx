...
// =============================================================================
// SubscriptionsPanel — external-feed subscription management (RSS / Nostr /
// Bluesky / Mastodon) via the omnivorous SubscribeInput, extracted so the
// workspace Glasshouse overlay (SubscriptionsOverlay) owns it. Mirrors
// SettingsPanel/LibraryPanel: a page-capable mode (`inOverlay=false`, with the
// auth redirect) is kept for the standalone /subscriptions route, but the
// overlay is the live surface inside the workspace.
// =============================================================================
...
export function SubscriptionsPanel({ inOverlay = false }: { inOverlay?: boolean }) {
  ...
  const body = (
    <>
      {inOverlay ? (
        <PageHeader title="External feeds" />
      ) : (
      ...
      {/* Subscription list */}
      <div className="mt-8">
        <div className="slab-rule-4 mb-6" />
  ...
  return <div className="mx-auto max-w-feed pt-8 pb-20 px-6">{body}</div>
}
