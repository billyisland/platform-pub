...
export default function SettingsPage() {
  ...
  if (loading || !user) return <div className="mx-auto max-w-lg px-6 py-12"><div className="h-7 w-20 animate-pulse bg-surface-raised mb-10" /><div className="h-28 w-full animate-pulse bg-surface-raised mb-8" /></div>

  return (
    <div className="mx-auto max-w-lg px-6 py-12">
      <h1 className="font-serif text-2xl font-light text-ink-900 mb-10 tracking-tight">Settings</h1>

      {!user.stripeConnectKycComplete && (
        <div className="mb-10 bg-surface-raised px-6 py-5">
      ...
      <section className="mb-10">
        ...
        {user.hasPaymentMethod ? (
          <div className="bg-surface-raised px-6 py-4"><div className="flex items-center justify-between"><div><p className="text-ui-sm text-ink-900">Card connected</p><p className="text-ui-xs text-content-faint mt-0.5">Your reading tab will settle automatically.</p></div><span className="text-ui-xs text-content-muted">Active</span></div></div>
        ) : (
      ...
      {user.stripeConnectKycComplete && (
        <section className="mb-10">
          ...
          <div className="bg-surface-raised px-6 py-4"><div className="flex items-center justify-between"><div><p className="text-ui-sm text-ink-900">Stripe Connect</p><p className="text-ui-xs text-content-faint mt-0.5">{user.stripeConnectKycComplete ? 'Verified — payouts are enabled.' : 'Verification pending.'}</p></div><span className="text-ui-xs text-content-muted">{user.stripeConnectKycComplete ? 'Verified' : 'Pending'}</span></div></div>
        </section>
      )}

      <section>
        <p className="label-ui text-content-muted mb-4">Account</p>
        <div className="bg-surface-raised px-6 py-4 space-y-3">
          <div className="flex items-center justify-between"><span className="text-ui-xs text-content-muted">Display name</span><span className="text-ui-sm text-ink-900">{user.displayName}</span></div>
          <div className="rule" />
          <div className="flex items-center justify-between"><span className="text-ui-xs text-content-muted">Username</span><span className="text-ui-sm text-ink-900">@{user.username}</span></div>
          <div className="rule" />
