...
28/94 into their 56-unit frame). It is narrower than the bare glyph's stance
...
trigger and the favicon in step.

As of 2026-07-25 the bare stance has no implementation left: the crimson ∀ that
carried it (Nav / Footer / About, `ForAllMark`) was retired from the design
language, so the DISC is the mark's sole rendered form and everything here is it.
The bare geometry survives as spec only, in docs/adr/LOGO-REFINEMENT-SPEC.md.
