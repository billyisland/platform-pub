...
**Status:** Phase 6B shipped (2026-05-25). Merged to `master` 2026-05-29. All phases complete. **Superseded in part (2026-06-02) by the _one post per card_ rule (see `CLAUDE.md` → Design system rules):** Phase 6B reply grouping (`groupReplies()` / `reply_group` / `ReplyGroupCard`) was removed — the gateway now emits each reply as its own item; the inline `ParentContextTile` on collapsed feed cards was removed (`PostCardInteractive` no longer renders it; parent context is reached by expanding into the thread). Also changed: the caught-up tile auto-dismiss is now **2s** (was 4s); the workspace feed has infinite scroll (the `sourceFilteredItems` cursor is consumed by `WorkspaceView`/`Vessel`); and at most one conversation stays expanded per feed. Post-ship fixes (2026-05-25): `PullToRefresh` accepts explicit `scrollRef` from `Vessel` (fixes default-height vessel pull-to-refresh per §8); `ParentContextTile` lifted out of `expanded` ternary in `ExternalVesselCard` (fixes §3.1 — parent tile now always visible for replies, not gated on expand). See `WORKSPACE-FULL-VIEW-DIAGNOSIS.md`. Phase 6B: reply grouping — pure `groupReplies()` post-processing in `sourceFilteredItems()` groups external items sharing the same `source_reply_uri` (2+ siblings) into `reply_group` envelopes; new `ReplyGroupCard` component renders `ParentContextTile` once + chronological `ExternalPlayscriptEntry` list. Phase 6A fixes: context-only items filtered from feeds, engagement counts added to platform timeline, grandparent tag persisted in `interaction_data` JSONB (both gateway parent-fetch and feed-ingest prefetch), missing `source_reply_uri`/`content_text` columns added to Mastodon prefetch INSERT, `WorkspaceFeedApiExternal` type completed (`contentWarning`, `poll`), `useLiveEngagement` cache aligned to 30s with error-recovery reset. Phase 5 adds: (5A) Mastodon content warnings — migration 093 adds `content_warning` column to `external_items`, AP adapter captures `spoiler_text` from sensitive notes, `ContentWarning` component wraps content with reveal toggle. (5B) Poll display + voting — AP adapter extracts `oneOf`/`anyOf` poll data into `interaction_data.poll`, `PollDisplay` component renders option bars with interactive voting, `POST /external-items/:id/poll-vote` endpoint enqueues via `enqueuePollVote`, feed-ingest `voteMastodonPoll` resolves remote status and POSTs to `/api/v1/polls/:id/votes`. (5C) Reader pane — `GET /api/v1/extract?url=` endpoint using `@mozilla/readability` + `jsdom` with SSRF-hardened fetch and 1h cache, `useReader` Zustand store, `ReaderPane` overlay component (scrim + 640px serif panel), RSS article clicks open reader pane. (5D) Inline video embeds — `MediaBlock` detects YouTube/Vimeo URLs, fetches oEmbed HTML from existing proxy on expand, renders iframe inline with `prefers-reduced-motion` respect. (5E) Pull-to-refresh + empty states — `PullToRefresh` component (touch + wheel overscroll, 60px threshold; desktop wheel accumulates upward delta at scrollTop 0 with 400ms decay), `EmptyFeedTile` with no-sources/no-items/caught-up variants, wired into Vessel and WorkspaceView; caught-up tile renders above existing items when refresh yields no new content; (2026-05-30) it also auto-dismisses ~4s after appearing if untouched via a `CaughtUpTile` subcomponent mount timer, paused on hover, with the manual ADD SOURCES / DISMISS / scroll-up paths still dismissing immediately. (5F) Context-only GC cron — `external_context_gc` task (daily 02:30 UTC) deletes unreferenced `is_context_only` items older than 30 days. Rich embeds (2026-05-30, CARD-BEHAVIOUR-ADR addendum): quoted posts render as nested `QuotedPostTile` mini-cards (author byline + text + the quoted post's own media), and external link previews render via `MediaBlock`'s `LinkPreviewCard` from `media[]` `{type:"link"}` entries. Quote hydration mirrors parent context — `external_items.source_quote_uri` (AP adapter `extractQuoteUri` covers FEP-044f / Fedibird / Misskey forms), eager `external_parent_prefetch`, and lazy `GET /external-items/:id/quote` with the standard rate-limit/cache/`partial` contract. Mastodon link cards are captured from the status `card` during `external_engagement_refresh`. Scope: Bluesky + Mastodon; Nostr/RSS deferred.
...
### Vessel UI batch — appearance controls, text size, media expansion, typography (shipped 2026-05-30)

A batch of vessel-surface refinements (build plan: `FEED-CHANGES-BUILD-PLAN.md`). Card-interaction details (media expansion, clickable external pip, shared byline, unified body size) are specced in the CARD-BEHAVIOUR-ADR.md "Media expansion, clickable external pip, unified body text & shared byline" addendum. The chrome/controls changes:

- **Appearance controls moved off the vessel bar into the composer modal.** `VesselBar` previously carried brightness (`○◐●`), density (`c/s/f`), and orientation (`|/─`) cycle buttons. These move into a new **"Appearance"** section in `FeedComposer` (opened via the gear, or numeral double-click), leaving the bar with only the gear, hide (`×`), and the add-source input. The feed numeral and resize grab stay on the chassis. The modal section renders four `AppearanceControl` cycle buttons in the `label-ui` idiom: Brightness, **View** (`Condensed / Standard / Full` — the density control, relabelled), Orientation, and **Text size**. (**Updated 2026-06-11:** the Brightness cycle button became the **colour-scheme swatch row** — `SchemePicker`, six curated schemes whose text colours are luminance-derived in `tokens.ts`; the scheme persists server-side as `feeds.appearance.scheme` (migration 112) with the layout store's `brightness` field as per-device cache. View / Orientation / Text size are unchanged. See CLAUDE.md › Workspace colour schemes + feature-debt §3.)
- **Per-feed text size.** A new `TextSize` (1–5, default 3) per-vessel setting (`tokens.ts`: `TEXT_SIZE_PX`, `nextTextSize`) persisted on `VesselLayout.textSize` via `useWorkspace().setVesselTextSize` (mirrors `setVesselDensity`, written to localStorage with the rest of the layout). The Text-size control uses two turned-A glyphs (`Ɐ`, U+2C6F) at 18px/12px with an `n/5` indicator; it is **orthogonal** to View (density). Step 3 = 13.5px keeps the prior body size. The value flows into cards via `CardContext.bodyPx` and governs all reading prose in lockstep; meta rows and bylines (mono `label-ui`) are unaffected.
- **Larger bar buttons.** The shared `BarButton` (gear/composer `⚙` and hide `×` on `VesselBar`) now renders at `BAR_H * 2` (64px) tall with a 22px glyph, flex-centered, instead of an 11px glyph filling the 32px bar via `lineHeight`. Because the button background is transparent, the extra height reads as a larger glyph plus a doubled tap target rather than a visible box overflowing the colored bar strip; the glyph stays vertically centered on the bar.

Touched: `web/src/components/workspace/{VesselBar,Vessel,FeedComposer,WorkspaceView,VesselCard,Byline,ParentContextTile,ExternalPlayscriptEntry,ExternalPlayscriptThread,ReplyGroupCard,tokens}.tsx` + `web/src/stores/workspace.ts` + `web/src/components/feed/AuthorModal.tsx`.
