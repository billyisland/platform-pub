...
**Status:** Phase 4C shipped (2026-05-24, branch `workspace-experiment`). `POST /external-items/:id/reply` endpoint: validates linked-account ownership + protocol match, signs kind 1 Nostr event via key-custody, creates note with `external_parent_id` + feed_items dual-write in one transaction, enqueues relay publish + outbound cross-post (atproto/activitypub via `enqueueCrossPost`, nostr_external via `enqueueNostrOutbound` with NIP-10 reply tags). `InlineReplyBox` component: inline text field below cards and playscript thread entries, platform badge, linked-account gating (hidden for RSS, greyed/prompt when no account), optimistic reply count. `EngagementRow` reply button interactive (speech bubble, same pattern as like/repost). `ExternalPlayscriptEntry` Reply tag wired to open InlineReplyBox per-entry. `ExternalPlayscriptThread` manages reply-to-entry state with linked account passthrough. `NoteVesselCard` renders `ParentContextTile` when note has `externalParentId`. Feed queries (`timeline.ts`, `feeds.ts`) pipe `n.external_parent_id` through to frontend types. Phase 5 pending.
