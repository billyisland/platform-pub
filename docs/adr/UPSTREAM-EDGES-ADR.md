...
> **⚠ Amended 2026-07-13 — Dial A adopted (consent-gated accrual). Supersedes Decisions 4 and 5 and the `held`/`swept`/`returned` lifecycle.** Nothing is apportioned or held until a tribute is `live` (consented + the inspirer is a real, onboarding account). Before consent there is **no `tribute_accruals` row** — the author keeps their full payable and sees only a *projection*; accrual is **forward-only** (frozen only on reads settling *after* the tribute goes live; pre-consent reads paid the author in full, never clawed back). This eliminates every held share for a non-consenting party, so `held`, `swept`, `returned`, the author swept-return columns, and the chain swept-return-to-parent plumbing are all retired — an accrual is only ever `released → paid` (plus `voided` on chargeback). Settlement-time apportionment for `live` tributes (Decision 3), the single `tribute_payout` entry (Decision 7), and the carve-at-payout mechanic are **unchanged**. Rationale + compliance effect (collapses the surface to baseline residual #1 alone; removes the funds-segregation collision): `UPSTREAM-EDGES-TRIBUTE-COMPLIANCE.md` › *Decision (2026-07-13)*. Code rework required before `TRIBUTES_ENABLED` flips: `UPSTREAM-EDGES-BUILD-PLAN.md` › *Dial-A rework*.
...
4. ~~**The inspirer's share is held until it resolves — money is never lost.**~~ **⚠ SUPERSEDED by the Dial-A amendment (2026-07-13).** *Under Dial A there is no held share for an unconsented tribute:* pre-consent reads pay the author in full and are never clawed back, so there is nothing to sweep back and no "always waiting for you" cushion. The share exists only once the tribute is `live`, and then only forward (see amendment). *(Historical model, retained for the record:)* ~~A held accrual is money the platform has collected but not yet attributed to a final owner. While held it reduces the author's payable. On consent → paid to the inspirer. On decline / window-lapse → swept back to the author's payable. The author is always the terminal home.~~ This is the same posture the platform **already** runs: an un-onboarded writer's earnings sit collected-but-unpaid in `read_events`/`platform_settled` ("earnings accrue and are held until verification completes") until they connect a bank. The held tribute share is the same idea, keyed to the tribute rather than an account.
5. ~~**Consent gates the destination of the held share, not whether it is apportioned.** Apportionment starts at tribute creation.~~ **⚠ REVERSED by the Dial-A amendment (2026-07-13): consent gates apportionment itself.** Apportionment starts at **consent** (tribute `live`), not creation — there is no held share before then. To *receive*, the inspirer must be (or become) a native account with completed Stripe Connect onboarding; consent + onboarding are now the gate on **whether the share is apportioned at all**, and accrual runs forward from that point.
...
```
The sum of a piece's live+proposed `percentage_bps` must leave the author a meaningful share — the build plan adds the cross-row ceiling (no per-row CHECK can express it). `proposed` = created, awaiting the inspirer (**under Dial A, 2026-07-13: NOT accruing — no accrual until `live`**); `live` = consented + onboarded (accrual starts here, forward-only); `declined`/`lapsed` = resolved against the inspirer (nothing was accrued, so nothing to return). An unreachable external inspirer stays `proposed` until the window lapses.
...
CREATE TABLE tribute_accruals (
  ...
  state text NOT NULL,                          -- Dial A (2026-07-13): released | paid (+ voided on chargeback). Historical (accrue-from-creation): held | released | paid | swept | returned | voided
  created_at timestamptz NOT NULL DEFAULT now()
);
```
This is the inspirer's suspense, and it lives **here, not in `ledger_entries`** — `ledger_entries.account_id` is `NOT NULL`, so (exactly as unpaid writer earnings sit in `read_events` until payout) the share sits in this domain table and only touches the ledger when it reaches a real account. **Dial-A lifecycle (2026-07-13):** a row is created only for a `live` tribute, in state `released` (the inspirer has consented + onboarded), → `paid` (transferred; `tribute_payout` ledger entry posted); `voided` on chargeback. One row per (live tribute, settled read). *(Historical accrue-from-creation lifecycle, superseded: `held` (accruing, reduces author payable) → `released` → `paid` | `swept`/`returned` (declined/lapsed → back to author).)*
...
C6. **The held-funds compliance position recurses unchanged — no new question.** **Under Dial A (2026-07-13) this clause is trivially satisfied and the swept-return-to-parent plumbing it describes is retired:** a child accrues only once *it* is `live` (C1), so there is never a held share for a non-consenting party at any depth, and a declined/lapsed child never accrued anything to return. What follows is the historical (accrue-from-creation) argument. Every held share is *some consented earner's* deferred earnings under a revocable redirect (the parent's), returned to that earner on decline/lapse — the identical structure `UPSTREAM-EDGES-TRIBUTE-COMPLIANCE.md` defends, now one level up the tree rather than always at the author. Because a child can be created only by a `live` (hence consented, hence earner) parent (C1), there is no held share whose nominal owner is not yet a confirmed earner. The memo should be re-read once with "the author" generalised to "the parent beneficiary," but it opens no new regulatory surface.
