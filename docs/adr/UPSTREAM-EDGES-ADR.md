...
- **Holding third-party funds — open compliance question.** Holding an un-onboarded *writer's* earnings is easy to defend (they signed up, accepted terms, chose to earn). Holding a share for a non-consented, possibly-not-yet-existing inspirer is closer to client-money / e-money territory. The mechanism is the same; the consent/identity seam is not. **Resolve with whoever owns compliance before build** — it shapes how long we may hold, what we disclose, and whether the suspense needs ring-fencing.
