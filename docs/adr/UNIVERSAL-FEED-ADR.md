...
**Atproto author identity (migration 137).** A Jetstream commit carries only the author's DID — no handle or display name inline — so the ingest cannot read per-post identity off the firehose. Since one atproto source = exactly one author (the listener matches commits to a source by DID), the account's handle is resolved once and stored on `external_sources.handle`, and the `feed_ingest_atproto_backfill` task enriches it (via `getProfile`) on subscribe. `insertAtprotoItem` then resolves the item's `author_name`/`author_handle` with a fallback chain: per-post profile (available on the backfill/poll `getAuthorFeed` path) → the source's stored handle/display_name → `@handle`. The failure this closes: the pre-137 ingest hard-coded `author_handle = NULL` and derived the name solely from `source.display_name`, so a Bluesky account with no display name (displayName is optional) produced a byline that fell through to the literal "External" (rendered uppercase as **EXTERNAL**). The backfill also **repairs** historical null-author rows (`external_items`/`external_authors`/`feed_items`) for the source, and the frontend byline now falls back to a protocol label ("Bluesky user", …) rather than a bare "External" as a floor.

**Enrichment is self-healing, not one-shot (2026-07-05).** Resolving the handle only at subscribe left a stuck population: sources subscribed before enrichment shipped, or where the subscribe-time `getProfile` failed transiently and never retried, kept `external_sources.handle = NULL`. Because the live listener attributes every commit from that stored handle, such a source mints null-author rows *indefinitely* — the byline sticks on the "Bluesky user" placeholder even though the account has a perfectly good handle. Fix: the Jetstream listener's 60s `refreshDids` (`feed-ingest/src/jetstream/listener.ts::enrichMissingHandles`) re-enqueues `feed_ingest_atproto_backfill` — whose first act is the `getProfile` enrichment + historical repair — for every active atproto source still missing a handle. It uses a distinct `job_key` (`feed_ingest_enrich_<id>`) so it can't clobber a fresh subscription's backfill, `add_job` dedupes so at most one enrichment per source is pending, and it is self-limiting: a resolved handle drops the source out of the null-handle filter on the next refresh, so a healthy fleet enqueues nothing. Transient `getProfile` failures now retry every 60s instead of never.

General rule for every ingest adapter: **persist per-post author identity (name + handle); never let a byline collapse to a placeholder as an identity.**
...
The universal resolver is the shared primitive. Each existing identity input across the platform should migrate to it over time. Current state (updated 2026-07-10 — resolver-audit F4 closed the DM/invite rows) and target:

| Feature                            | Status                                                                            | Location                                                  | Target / as-built                                                                                                                                                                                         |
| ---------------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Feed subscribe**                 | **Adopted** (Phase 1)                                                              | `FeedComposer.tsx` / `VesselBar.tsx` (`useResolverInput`) | Full resolver: URLs, handles, npubs, DIDs, NIP-05, fediverse handles, RSS URLs — plus the §V.5.8 discovery fallback on submit.                                                                              |
| **Publication invite**             | **Adopted 2026-07-10** (audit F4; was resolver-via-hand-rolled-debounce)           | `MembersTab.tsx`                                          | `useResolverInput` context `invite`: a single match resolves implicitly, multiple matches need an explicit pick, no-match falls back to invite-by-email. The hook's `genRef` closed the stale-match race.   |
| **DM new conversation**            | **Adopted 2026-07-10** (audit F4; was username-only `results[0]`)                  | `MessagesInbox.tsx`                                       | `useResolverInput` context `dm` (native-only server-side): matches render as rows, the conversation starts on a *picked* account; Enter auto-picks only an unambiguous single match.                        |
| **DM pricing override**            | **Adopted 2026-07-10** (audit F4; was username-only `results[0]`)                  | `DmFeeSettings.tsx`                                       | `useResolverInput` context `dm`: pick into a clearable person chip; Add is disabled until someone is selected.                                                                                              |
| **Search**                         | Open (free-text query)                                                             | `web/src/app/search/page.tsx`                             | Add resolver as a pre-pass: if the query looks like an identifier (URL, handle, npub), resolve first and show the resolved result above search results.                                                     |
| **Publication ownership transfer** | Open (UUID only)                                                                   | `gateway/src/routes/publications/members.ts`              | Username, email, npub. Resolve to native account (must be platform member).                                                                                                                                 |

Person-picking surfaces (DM, invite, fee override) act on the account rather than a feed-source add via the additive `MatchOption.account` identity (`web/src/lib/workspace/resolve.ts::matchToOptions`, carried from `native_account` matches). Remaining migrations (search pre-pass, ownership transfer) are incremental, self-contained changes onto the same hook. Log: FIX-PROGRAMME 2026-07-10.
...
> **Extended by `RESOLVER-DISCOVERY-ADR.md` (accepted 2026-07-10; all built phases shipped 2026-07-10).** The invariants below (submit-only, context-gated, capped, nomination-not-resolution) stand unchanged; the branch table is superseded: two new branches landed (known-world pg_trgm index, Phase A; `activitypub_discovery` behind a FASP-ready provider interface), branch 3's catalog grew from the 11-entry seed to 500 generated probed-live entries (`scripts/gen-discovery-catalog.ts`), the merge step became bridge-aware with confidence-tier ordering, and the tiers render as Matches/Suggestions. Branch 4 remains deferred behind `DISCOVERY_WEBSEARCH_ENABLED` as specified there (§7.2).
...
| - | ------------------------------- | ---------------------------------------------------------------------- | --------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | **Bluesky actor search**        | `app.bsky.actor.searchActors` on the public AppView (no auth)          | low       | high      | Purpose-built typeahead. "Guardian" → real candidate accounts in one fast call. Reuses the existing atproto resolve plumbing. **Shipped 2026-06-08.** |
| 2 | **Nostr name search**           | NIP-50 (`{kinds:[0], search}`) on a search relay (default `wss://relay.nostr.band`, `NOSTR_SEARCH_RELAY`) | low       | medium    | Free, partial-friendly. Returns npub candidates that re-enter the npub chain. **Shipped 2026-06-08.** |
| 3 | **Curated publication catalog** | Local seed table: common name → canonical feed URL (Guardian, BBC, NYT…) | instant   | very high | Covers the head of the distribution cheaply. Low recall by design — one precise branch among several, not the whole answer. **Shipped 2026-06-08.** |
...
**Phasing.** Branch 1 (Bluesky `searchActors` on the empty/`free_text` path) was the self-contained first slice — small, no new dependencies, immediately makes names and typo'd handles produce pickable candidates. Branches 2–3 followed incrementally (shipped 2026-06-08); branch 4 ships last behind a feature flag once the cheap branches prove the seam.

**Implementation status (branches 1–3, shipped 2026-06-08).** The submit-only guardrail is enforced server-side by a `discover: boolean` flag on `POST /api/resolve` (default `false`): only an explicit submit (Enter) sets it, so the debounced-keystroke typeahead never triggers external search. With `discover: true`, the `free_text` case and the no-exact-hit `platform_username` branch register the discovery Phase B chains (`catalog_discovery`, `bluesky_discovery`, `nostr_discovery`), all gated identically (invite/dm never discover). All three emit `speculative` matches deduped within their own protocol's key-space, with no schema change — reuses `resolver_async_results` (migration 061) and the existing `speculative` confidence.

- **Branch 1 (Bluesky)** — `atproto-resolve.ts::searchActors` (public AppView, hardened `safeFetch`) → atproto `external_source` matches deduped by DID.
- **Branch 2 (Nostr)** — `resolver.ts::searchNostrProfiles` runs a NIP-50 `{kinds:[0], search}` query over a temporary SSRF-pinned WebSocket to the search relay (`NOSTR_SEARCH_RELAY`, default `wss://relay.nostr.band`), newest kind-0 per pubkey wins; emits `nostr_external` matches deduped by hex pubkey. Kind-0 content parsing is shared with single-profile enrichment via `parseNostrProfileContent`.
- **Branch 3 (catalog)** — `discovery-catalog.ts::searchCatalog` is a pure, zero-I/O seed table (name aliases → canonical feed URL) → `rss_feed` matches. Resolved synchronously and persisted first so the head case renders before the network branches.
...
**Profile metadata (kind 0):** The per-poll ingest keeps the newest kind-0 on `external_sources` (`display_name`, `avatar_url`) and now also persists the verified **nip05** to `external_items.author_handle` — the `feed_items` identity trigger propagates it to `external_authors.handle`, so the card byline + hover bio render the verified `@handle` (Nostr has no handle@host). Existing authors backfill on their next post; no migration. The hover bio additionally reads kind-0 **live** at render time (`gateway/src/lib/nostr-relay.ts::fetchNostrAuthorProfile`, source relay hints → fallback aggregators) to surface `about` (bio), `website`, and `lud16` (⚡ lightning address) — the analogue of Bluesky/Mastodon's live profile fetch, since Nostr has no profile REST API. Follower/post counts aren't cheaply countable on relays, so stats stay absent (§4.4 "no stats ⇒ show none").

**Engagement counts (reactions + replies):** The base poll filters on `authors:[pubkey]`, so it never sees engagement — reactions (kind 7) and replies (kind 1) come from *other* pubkeys. `external_engagement_refresh` (B6) handles Nostr alongside Bluesky/Mastodon, gated behind `NOSTR_ENGAGEMENT_COUNTS_ENABLED` (relay REQ latency is the heaviest source, so it ships dark under its own tighter per-run cap, `NOSTR_MAX_ITEMS`, separate from the HTTP-platform budget). It fetches kind-7 / kind-1 events that `#e`-tag the source's recent notes (`feed-ingest/src/lib/nostr-relay.ts::fetchNostrEngagementCounts`, hint relays → fallback aggregators, id-chunked), tallies per note, and writes `external_items.like_count`/`reply_count` **monotonically** (`Math.max` against the stored value, so a partial relay read can only raise a count, never flicker it down). NIP-25 `"-"` reactions are excluded; reposts stay 0 (kind 6/16 are edges, and nostr cards hide repost); zaps are out of scope (no count column yet). `PostCounters` renders ♥/↩ from these for `nostr_external`.
...
- **Full sitewide resolver adoption.** The universal resolver (§V.5) ships in Phase 1 and the publication invite flow is migrated immediately. ~~Downstream adoption (DM flows, search pre-pass, DM pricing overrides, ownership transfer) is incremental~~ — DM flows, DM pricing override, and the invite's proper `useResolverInput` wiring landed 2026-07-10 (resolver-audit F4); search pre-pass and ownership transfer remain open. Each remaining migration is a small, self-contained change onto the same hook. See §V.5.5 for the adoption table (statuses maintained there).
