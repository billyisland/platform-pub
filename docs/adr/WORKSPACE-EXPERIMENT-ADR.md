...
- **Orientation icon depicts the vessel.** The glyph was a bare `|` / `─`. It is now an `OrientationGlyph` SVG — a tall portrait rectangle for `vertical`, a wide landscape rectangle for `horizontal` (2px stroke, fixed 16×16 viewBox), so the button shows the *shape of the feed container* it sets. The fixed `104×34` `AppearanceControl` footprint from slice 36 is retained, so cycling still never reflows.
