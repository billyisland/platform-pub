...
- **Clipped to the disc, flush on the rim.** The glyph span is replaced by an inner `<svg viewBox="0 0 56 56">` (`position:absolute; inset:0`); a `clipPath` of the disc circle (`cx/cy 28, r 28`) terminates the diagonals flush on the circumference so the segments are complete. Because the bars are the floor colour, they read as the ground cutting through the disc; the **black segments** they carve out (not a black ring) are what mark the disc against the surrounding ground. The SVG is an unclipped sibling of the unread badge. Coordinates: left diagonal `(28,56)→(8.5,5)`, right diagonal `(28,56)→(47.5,5)` (the upper endpoints sit just outside the disc so the clip lands them on the rim), crossbar `(18.1,30)→(37.9,30)` (endpoints computed to land on the diagonals at `y=30`).
