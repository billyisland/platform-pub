...
- **Inline SVG, clipped to the disc.** The glyph span is replaced by an inner `<svg viewBox="0 0 56 56">` (`position:absolute; inset:0`). A `clipPath` of the disc circle (`cx/cy 28, r 28`) clips the bars so their outer ends sit **flush on the rim** — that flush-to-circumference termination is the point (the bars *define* the segments), and because the bars are solid strokes on black with no rim slivers of a clipped letter, there's no apparent outline. Coordinates (the diagonals' splay was tuned across a few rounds — a wide first cut, an over-steep second, then split halfway): left diagonal `(28,56)→(8.5,5)`, right diagonal `(28,56)→(47.5,5)`, crossbar `(18.1,30)→(37.9,30)` (round cap; endpoints computed to land on the diagonals at `y=30`). The SVG stays an inner sibling so the unread badge remains unclipped on the button.
- **White.** `stroke={TOKENS.glyphFg}` (`#FFFFFF`). The button's `font-serif` class is dropped (no text to set a face on).
- **Tuning knobs.** Diagonal rim points (`8.5,5` / `47.5,5`) set the splay; the shared `(28,56)` is the bottom point; crossbar `y` (30) and its leg-intersection x's (`18.1`/`37.9`) set its height. `strokeWidth` sets bar weight.
