...
- All four item handlers are `console.log` stubs for this slice. Wiring lives in later slices: _new note_ → `Composer` (per ADR §"Adapted"), _new feed_ → `POST /api/v1/feeds` (slice 3), _fork by URL_ → universal resolver, _reset layout_ → `useWorkspace` (slice 4).
...
### Slice 2.5 — minimal Composer wired to _New note_ (2026-05-01)

`web/src/components/workspace/Composer.tsx` is the workspace's own composer surface — fresh component, _not_ a reuse of the retired `ComposeOverlay`. Slice 2.5 ships note mode only.

- Centred panel over a 40% scrim, opens on ∀ → _New note_. Esc / scrim click / Cancel close it.
...
- To-field resolution and protocol selection are **not** wired this slice. Typing into To disables Publish and surfaces a hint that private addressing arrives in a later slice. This honours the spec's invariant — that the To field is _the_ central narrowing gesture — without faking autocomplete.
...
`ForallMenu` becomes controlled — it now takes `onAction(key)` rather than owning stub handlers. The other three actions (_new feed_, _fork by URL_, _reset layout_) remain stubs in `WorkspaceView.handleForallAction`.
...
**To field.** Composer-local input now wraps a chip row. Typing 300ms-debounce-resolves through the universal resolver (`POST /api/v1/resolve`, context `dm`) and Phase B polls (up to 3 ticks at 1s) — same shape as `SubscribeInput` but adapted for chip selection. Native account matches surface in a dropdown alongside fuzzy-matched broadcast tokens (_Everyone on all.haus / Nostr / Bluesky / fediverse_). Enter on the input adds the top person match; click adds either kind. Backspace on an empty input pops the last chip; per-chip × button removes any chip. Person chips render black, broadcast chips render light.
...
- Person chips present → publish disabled with hint _Private addressing wires in a later slice — remove person chips to publish_. The DM/private-addressing pipeline is its own slice (the resolved chip carries the protocol, but encrypted DM dispatch + cross-protocol DM is non-trivial).
- Empty To with Nostr toggled off, or broadcast-only To without Nostr → publish disabled with hint _Cross-protocol broadcast wires later. Include Nostr to publish._ The hint is honest about what the existing `publishNote` pipeline does (it signs and outbox-enqueues a Nostr kind 1; cross-posting to ActivityPub / atproto requires `crossPost` + a `linked_account_id`, deferred).
- Otherwise (empty To with Nostr on, or broadcast-token-list-including-Nostr) → public publish via existing `publishNote(content, user.pubkey)`. Other-protocol toggles and other-protocol broadcast tokens are accepted by the UI and ignored at publish time; the gating hint and the _Send via_ selector make this state visible.
...
- _Person chips only_ → `POST /conversations { memberIds }` then `POST /messages/:conversationId { content }`. The existing `gateway/src/services/messages.ts` handles NIP-44 encryption end-to-end via key-custody — the composer doesn't see plaintext leaving the browser any differently than the existing `/messages` page does. Multiple person chips form a group conversation in one call.
- _Empty / broadcast-only_ → unchanged from slice 2.6 (public `publishNote` via Nostr).
- _Mixed_ (person chips + broadcast tokens) → publish disabled with hint _Mixing people with broadcast targets isn't supported in one send_. Two intents in one gesture is genuinely ambiguous; one-or-the-other is the cleanest contract.

**Surface changes.**

- _Publishing publicly_ banner is suppressed when any person chip is present (the publish is no longer public).
- Action button label flips: `Publish` → `Send` for private. `Publishing…` → `Sending…` while in flight.
- Hint reads _Sending privately to N recipient(s) — appears in their inbox at all.haus/messages._ — naming the destination so the user understands the gesture's outcome.
- DM-pricing skip handling: if `messages.send` returns a non-empty `skippedRecipientIds`, surface _Sent, but N recipient(s) were skipped — DM pricing not paid_ and leave the composer open. Settling DM pricing happens via the existing `/settings/dm-pricing` flow; not in scope here.
...
The protocol toggles and "Everyone on Bluesky / fediverse" broadcast chips light up. A public publish now anchors on the native Nostr publish _and_ fans out to the user's connected Bluesky / Mastodon accounts, on the back of the existing Phase 5 outbound pipeline.
...
- On open, `Composer` calls `linkedAccounts.list()` and bins valid accounts by protocol. The atproto + activitypub toggles in the _Send via_ row reflect connection state: connected → toggleable as before; not connected → rendered disabled-grey with a `title` reading _Connect &lt;Bluesky/Activitypub&gt; in Settings → Linked accounts to broadcast there_. Native `ALL.HAUS` and `NOSTR` toggles are unaffected (always-on; `nostr` still gates the publish).
...
- Hint copy: when at least one cross-post target is queued, the char-count line becomes `Publishing to Nostr · BLUESKY · ACTIVITYPUB — N/1000`. The slice 2.6 _Cross-protocol broadcast wires later_ hint is gone; the still-present _Include Nostr to publish_ gate (Nostr-toggled-off broadcast) reads as _Cross-protocol broadcast needs Nostr as the anchor. Include Nostr to publish._ — matching the new reality that the _other_ protocols _are_ wired but Nostr remains the anchor.
...
- `DELETE /api/v1/feeds/:id/sources/:sourceId` — straight delete. The associated `external_subscriptions` row is deliberately _not_ torn down: a user may keep the subscription via `/subscriptions` or use the same external source in another feed. Subscription teardown is its own gesture.
...
**Composer (`web/src/components/workspace/FeedComposer.tsx`).** New component, scrim/panel grammar matching `Composer` and `NewFeedPrompt`. Reached by clicking the vessel name label (`Vessel.tsx` gained an `onNameClick` prop — long-press lives in the gesture system not yet built). Shows the feed name as header, lists current sources with × remove buttons, and offers an "Add a source" input that resolver-debounces (300ms + Phase B polling, context `subscribe`) and renders match candidates (native account / external source / RSS feed). Click a candidate → POST → list refreshes → `onSourcesChanged` triggers `loadVesselItems` on the affected vessel. Tag fallback: input starting with `#` offers a literal `tag` add when the resolver returns nothing useful (the resolver doesn't classify `#tag` natively). Honest empty state: _No sources yet — this feed shows the explore stream until you add one._
...
**Delete (footer).** A new bottom row separated by hairline, with a single `Delete feed` mono-caps button at right (grey → crimson on hover). Click → swaps in-place to a two-step confirm row: hint _Delete this feed? Sources are removed; subscriptions are kept._ + `Cancel` + crimson `Delete`. The two-step in-panel confirm is lighter than `ResetLayoutConfirm`'s modal because the action is feed-scoped (one row to undo by re-creating) rather than workspace-scoped.

**Last-feed guard.** `WorkspaceView` passes `deleteBlocked={vessels.length <= 1}`; in that case the footer renders a hint reading _Can't delete your only feed — create another first._ in place of the delete button. Without the guard, a sole-feed delete would leave the floor visibly empty until the next bootstrap reseeded a default — an awkward hidden recovery path. The gateway `DELETE /workspace/feeds/:id` would happily delete it; the FE-only guard is the friction layer.

**Subscription preservation.** The composer's hint copy (_subscriptions are kept_) names the deliberate behaviour from slice 4: deleting a feed cascades to its `feed_sources` rows but leaves any underlying `external_subscriptions` rows intact. The user can keep the subscription via `/subscriptions` or reuse the source on another feed.

**Layout cleanup.** On delete, `onDeleted(feedId)` drops the vessel from `vessels` _and_ calls `useWorkspace.removeVessel(feedId)` so the localStorage layout entry doesn't accumulate stale records. The store method already existed from slice 5a; this is its first wired caller.
...
### Slice 8 — ∀ → _Fork feed by URL_ wired (2026-05-01)

The third ∀ stub goes live. _Fork feed by URL_ is a one-gesture combo of _create feed_ + _add first source_ + _open vessel_, sharing the universal-resolver input grammar from `FeedComposer`'s "Add a source" but minting a fresh feed each time.

**Surface (`web/src/components/workspace/ForkFeedPrompt.tsx`).** New component. Same scrim/panel grammar as `NewFeedPrompt` (40% scrim, 480px panel, hairline black border, 144px top inset). Single input — _Paste a URL, @username, npub, DID, or #tag_ — that resolver-debounces (300ms + Phase B polling, context `subscribe`). Match candidates render below as a list of mono-caps-sublabelled buttons; clicking one performs the fork. Tag fallback for `#name` inputs mirrors `FeedComposer`. Hint copy under the input names the gesture's outcome: _Picks something below to mint a new feed pointed at it. Rename later from the feed composer._

**Fork mechanics.** `handleFork(opt)` runs `workspaceFeedsApi.create(derivedName)` then `workspaceFeedsApi.addSource(feedId, opt.add)` in sequence. The derived name comes from the resolved match — display name → @username → URI → feed title — clamped to 80 chars. If `create` succeeds but `addSource` fails, the partial feed is _kept_ and handed back via `onForked(feed)`; the modal surfaces a hint reading _Feed created but source add failed: …_ so the user can finish wiring it via the feed composer rather than losing the new vessel. Roll-forward over rollback because the feed itself is salvageable state.
...
**Why "fork" not "subscribe".** The menu copy uses _fork_ because the verb in this branch's vocabulary is workspace-floor-shaped: _fork_ makes a new vessel from an external thing the way a software fork makes a new repo from a remote one. Subscribing to a single source from `/subscriptions` (which still exists as the Phase 1–4 surface) is a different gesture with a different mental model.

Skipped intentionally: multi-source fork (the menu item is "Fork _feed_ by URL"; the user mints one source at a time and adds more in `FeedComposer`), fork from clipboard (browser permission costs > value at this fidelity), recently-resolved suggestions (the resolver doesn't expose a history surface yet), import a feed-of-feeds (e.g. an OPML upload — out of scope for the workspace shell), in-place rename of the derived name _before_ the fork commits (the modal's hint already promises rename-later via the feed composer; an inline rename would slow the gesture to two steps when one of the resolver matches is good enough).

### Slice 9 — ∀→H→⊔ ceremony animation (2026-05-01)

The signature visual move per `WORKSPACE-DESIGN-SPEC.md` §"The ∀-to-H-to-⊔ transformation" + `WIREFRAME-DECISIONS-CONSOLIDATED.md` Step 9. Two paces: _ceremonial_ on first-login (~3000ms, terminal state populated with card placeholders) and _responsive_ on each new feed creation (~1300ms, terminal empty ⊔). Framer Motion's `AnimatePresence` + transformable SVG primitives enter the codebase for the first time, on the back of the slice 5a `motion.div` baseline.

**Component (`web/src/components/workspace/ForallCeremony.tsx`).** A floor-relative absolutely-positioned 300×300 SVG overlay. Five phases driven by `setTimeout` boundaries off `CEREMONY_TIMINGS[pace]`: `forall` → `partingToH` → `hHold` → `crossbarDrop` → (`cards`, ceremonial only) → `done`. The ∀ glyph renders as Literata text inside the SVG, crimson `#B5242A`, scales 0.4→1 from box centre, then fades as the H bars resolve. Verticals fade in (matching the ∀'s exit), the crossbar enters at H position (mid-Y, between the verticals), then animates `(x, y, width)` to the ⊔ base position (full-width, bottom). For ceremonial pace, three white card placeholders fade in inside the resolved ⊔. `transition` uses custom cubic-bezier curves: `[0.33, 0, 0.15, 1]` on the crossbar drop (starts with a slight lag, resolves decisively) and `[0.25, 0.1, 0.25, 1]` on entries (gentle ease-in-out with smooth deceleration).
...
**Timings (`web/src/lib/workspace/motion.ts`).** New `CeremonyTiming` interface + `CEREMONY_TIMINGS` map. Ceremonial sums to ~2990ms (`forallIn 280 + forallHold 200 + partToH 260 + hHold 1000 + crossbarDrop 500 + cardsSnap 450 + settle 300`). Responsive sums to ~1300ms with `cardsSnap = 0`. The `hHold = 1000` honours the spec's "the slowest moment" framing — the H is held long enough to register as a deliberate pause.
...
3. Append vessel + write position to the layout store _immediately_ — items fetch starts behind the curtain
...
`Composer` becomes the single composing surface for both notes and articles, per `WIREFRAME-DECISIONS-CONSOLIDATED.md` Step 6. Note mode (slices 2.5–2.8) is unchanged; article mode adds a TipTap-backed editing surface, title + dek + publication selector + paywall toolbar, and the 400-word elevation nudge. Two entry points: ∀ → _Write an article_ (direct), and the in-composer _Write an article →_ link (or 400-word nudge) from note mode (elevation).
...
3. _Publish as_ selector — `<select>` defaulting to `PERSONAL`. Populates from `publications.myMemberships()` pre-fetched on open. Memberships without `can_publish` annotate the option label `(review)` and the publish button flips to `Submit for review`.
...
**Hint copy + button.** Bottom row in article mode reads `N words · M min read[ · Saved]`. Person chips in the To field disable publish with _Articles can't be sent privately — remove person chips to publish._ The Publish button is crimson `#B5242A` in article mode (matches the wireframe's _publish button turns crimson_); label flips to `Submit for review` for memberships without `can_publish`.
...
**Cross-protocol broadcast in article mode.** Hidden — the _Send via_ row only renders when `mode === 'note' && chips.length === 0`. The article path always anchors on Nostr (kind 30023) with no atproto/activitypub fan-out. Cross-posting articles to ActivityPub or Bluesky is its own slice (the Bluesky/Mastodon outbound paths key on a Nostr kind-1 source event; articles are kind 30023 and would need their own routing through `outbound_posts.action_type = 'original'` plus a per-protocol body shape decision — defer until users actually ask for it).

**400-word nudge.** New `web/src/components/workspace/Composer.tsx` local state `nudgeDismissed` + `showNudge`. While in note mode, an effect counts whitespace-split words and shows an inline panel reading _This is getting long. Switch to article mode?_ with `Switch` (crimson) and `Dismiss` (grey) buttons. Threshold is 400 words per spec. Dismissal is per-Composer-session (resets on close/reopen). The nudge is a one-shot panel rather than a recurring toast — once dismissed, it stays gone for the rest of the open session even as the user keeps typing.

**∀ menu fifth item.** `ForallMenu` adds `'new-article'` after `'new-note'` with label _Write an article_. `WorkspaceView.composerOpen` becomes `false | 'note' | 'article'` so the same Composer instance can open in either mode. Mode resets on open via the `initialMode` prop.

**`/write` page survives.** The route still serves the long-form editor (full toolbar, tags, scheduling, edit-published-article via `?edit=`, draft resumption via `?draft=`). It's no longer the _only_ way to write an article — the workspace Composer covers fresh-publish + paywall-gate + publication routing for the common case — but the Migration Map's "undecided" verdict on `/write` resolves provisionally as **survives as deep-link form** (per Open Item §5.6).

Skipped intentionally: schedule button (no draft-then-schedule UI in the panel; falls back to `/write?draft=…` if the user wants to schedule), tag input (Wireframe Step 6 doesn't list it among article-mode zones; tags survive on `/write`), comments toggle, _show on writer profile_ toggle for publication articles, edit-published-article from the workspace (the panel only knows fresh-publish; editing routes to `/write?edit=…`), draft resumption (the panel always opens fresh — opening a saved draft routes to `/write?draft=…`), embed toolbar button (simpler IMG-only toolbar this slice), price suggestion based on word count (the legacy `/write` has it; defer until the panel sees real use), publish-confirmation panel with email-subscribers checkbox (slice goes straight from Publish click to publish; the confirm/email flow is per spec but adds a step that didn't fit the workspace's _gesture is the publish_ feel — revisit), back-to-note from article mode (one-way elevation), TipTap-as-note-mode (the textarea stays — note→article is a real mode change with content carry-over, not a chrome change over a single editor), 760px article-mode width per the legacy ALLHAUS spec (kept at 640 for both modes; revisit if the article surface feels cramped). Cross-posting articles to ActivityPub / Bluesky and the article→DM "private article" gesture remain explicitly out of scope.

### Slice 11 — card click-through + action strip + reply context (2026-05-01)

Vessels stop being read-only display surfaces. Cards click through to the reader, gain a quiet `vote · reply · share` strip under the body in standard / full density, and the _Reply_ gesture finally gives `Composer` its second open-the-composer entry point. The `useWorkspaceCompose` extraction the slice-2.5 build log forecasted is _not_ built — `WorkspaceView`-local state (`composerOpen` + `replyTarget`) is enough for the two entry points (∀ menu and card-action-strip Reply); a store extraction can wait until a third entry point arrives.
...
- The `Publishing publicly` banner + To-field + protocol selector + _Write an article →_ link are all hidden. The cleanly-removed surface is replaced with a _Replying to NAME — excerpt_ header in the slice-10 banner-bg grammar (mono-caps name, italic Literata excerpt).
...
- Char limit stays at 1000; reply-only `parentCommentEventId` (for nested-thread context) is _not_ supported this slice — only top-level replies to a card. Threaded replies on the workspace are their own slice (the playscript thread render pattern from `web/src/components/replies/PlayscriptThread.tsx` survives per Migration Map §2 _replies/_ but isn't yet wired into the vessel surface).

**Wiring (`WorkspaceView.tsx`).** New `replyTarget: ReplyTarget | null` state. The ∀ menu's _new note_ / _new article_ paths clear it; the card action strip's `onReply` sets it + opens the composer in note mode. `Composer.onClose` clears both `composerOpen` and `replyTarget` so the next ∀ → _New note_ doesn't accidentally inherit a stale reply context.
...
The TrustPip stops being inert. Tapping the pip on a native (note / article) card opens a popover with header, bio, a first-cut trust signals block, and a SUBSCRIBE footer when the writer offers subscriptions. Per `CARDS-AND-PIP-PANEL-HANDOFF.md` §"The pip panel" the panel is the _judgment + commitment_ surface — slice 12 ships _commitment_ (FOLLOW + SUBSCRIBE link) in full and ships _judgment_ as a placeholder until the polling backend exists.
...
**TRUST section (first cut).** Renders the existing `trust_layer1` signals — `accountAgeDays`, `articleCount`, `payingReaderCount`, `nip05Verified`, `paymentVerified` — as a five-row mono-caps right-aligned-value list. Honest about what's missing: a small Literata italic line below reads _Polling questions land in a future slice._ The handoff doc's three poll questions (_Are they human? · Are they who they seem to be? · Do they engage in good faith?_) need a polling backend that doesn't exist (`docs/adr/ALLHAUS-OMNIBUS.md` §III.7 frames this as trust-system-spec-proper territory). Layer 1 signals are a real-data surface for the same question — _what does the system know about this author?_ — and ship in advance of the polling system rather than blocking the panel's other elements.

**VOLUME section.** Skipped this slice. The handoff calls for a five-step horizontal volume bar with a RANDOM/TOP sampling toggle — both surface and persistence. Per ADR §3 _Saved items: defer. Stub in code, no schema until the surface design solidifies._ Volume settings are in the same category — they're per-user-per-author state, no schema, and the bar's interaction model needs a proper design pass (continuous drag vs discrete steps; the touch gesture deferred per ADR §5; the `RANDOM` vs `TOP` semantics from the handoff's open architectural questions). Slice 12 leaves the section out entirely rather than ship a non-functional placeholder; it returns when the schema lands.
...
**State (`WorkspaceView.tsx`).** Two new pieces: `expandedThreads: Set<string>` (which event ids are open) and `threadRefreshTicks: Record<string, number>` (per-target counter bumped after an overlay-Composer reply lands). Toggle handler flips membership; the slice-11 overlay Reply path stays as the _fast_ compose surface, but its reply-publish now also auto-expands the affected card's thread and bumps the tick so the new reply is immediately visible.
...
- `DELETE /workspace/feeds/:id/author-volume/:pubkey` — clears the row (back to passive). Unknown-author DELETE returns 204 rather than 404 because the only client gesture is _clear commitment_, and a missing row already represents the cleared state.
...
**Question naming choice.** The handoff intentionally distinguishes the three poll questions from the four `vouches` dimensions (humanity, encounter, identity, integrity). `humanity` overlaps; `authenticity` ("who they seem to be") is _deliberately weaker_ than the formal `identity` vouch dimension; `good_faith` is the behavioural-honesty question the handoff explicitly defends as _not_ the abstract `integrity`. New names rather than reusing existing dimension labels keep the two surfaces distinct in the schema, which matters when the anonymous-attestation rewrite arrives — vouches and polls have different anonymity guarantees.
...
**Pip-colour composition still deferred.** The handoff §"Trust section" + §"Open architectural questions" notes that the pip's four-state colour (green / amber / grey / crimson) should compose from the three poll results plus in-person (`encounter` vouch) count. Slice 15 ships _the data and the gesture_ but doesn't change the pip's colour mapping — `trust_layer1_refresh` still drives `pip_status` purely from Layer 1 signals. Wiring poll aggregates into the pip mapping is a separate slice that owns the threshold function (the handoff calls it out as trust-system-spec-proper territory).
...
The slice 14 volume bar stops being a placebo. `sourceFilteredItems` now ranks rows by an `effective_score = mode_value × weight` blend rather than chronological-ignoring-weight. The volume bar's hint copy in `PipPanel.tsx` drops the _ranking by volume lands when the items query honours weight_ admission and reads the terse _Weight applied to this feed's ranking._ — the bar's commitment is now observable in the vessel that triggered it.
...
**No pip-panel surface change.** `PipPanel.tsx` already shows raw L1 + polling aggregates; the composition lives behind the inline pip glyph. The panel surfaces the _constituent_ data so users can see why a pip is the colour it is — no new copy needed.
...
**Threshold call.** Encounter ≥ 1 = anchor, ≥ 2 = strong-alone. Polling has a `SAMPLE_FLOOR = 3` because polls are cheap and Sybil-prone; encounters are deliberate, expensive gestures (you have to know the person _and_ explicitly affirm you've met them) so the floor is lower. A single affirm doesn't lift you above grey on its own (still requires polls + anchor for green, or strong L1 for amber), but it does _gate_ the green path. Two affirms gates amber. The constants `ENCOUNTER_ANCHOR = 1` / `ENCOUNTER_STRONG = 2` live at the top of the module so a tuning pass touches one place.
...
Skipped intentionally: encounter-public visibility from the panel (the full-vouch surface at `/network` still owns public-encounter affirms; the panel's gesture is the lightweight aggregate path), confidence-weighted encounter (the handoff alludes to attestor weighting per Layer-2 epoch aggregation — that's the trust-system-proper rewrite, not this slice; here every affirm counts equally), encounter-as-poll-question hybrid (the handoff considers a "did you meet them?" yes/no question alongside the four poll questions — different mental model, separate slice), per-viewer pip composition that _requires_ an encounter from someone in your follow graph (Layer 4 in spirit; needs Layer 4 plumbed into the per-request compute path), encounter requiring two-sided confirmation (the spec's eventual "we both met" reciprocal flag — current implementation is one-sided like the rest of `vouches`), encounter on external authors (no platform user id; the `subject_id` foreign key keys on `accounts.id`), seeding a refresh on encounter-write so freshly-vouched subjects update the pip faster than the daily cadence (matches the slice-15 / slice-17 pattern), pip-panel "remove I've met them" confirm step (one-tap toggle is the right friction floor — the user can re-tap to revert).
...
- `known` → `#1d9e75` · _Established profile — readers confirm the basics._
- `partial` → `#ef9f27` · _Developing profile — some signal, more would help._
- `unknown` → `#b0b0ab` · _New here — tap below to share what you know._
- `contested` → `#B5242A` · _Contested — readers have raised concerns._
...
**Subtitle under the writer name.** Italic Literata 13px in `TOKENS.hint`, sits between the header row and the bio (or directly above the TRUST section when bio is empty). Names what the pip means in plain language, and for non-green states gestures toward the affordance that exists below — _tap below to share what you know_ points at the polls + encounter sections without spelling them out.

**Self-pip suppresses both.** New `showFraming = !isOwn && !loading && !error && writer !== null` gate. The framing is about how _others_ read the writer; doesn't apply when looking at your own pip. The load/error gating prevents a flash of "wrong colour" stripe before the trust profile loads — the panel currently assumes a default of `unknown` until trust data lands, which would otherwise render a grey stripe for ~150ms before flipping to the real status.

**No section reordering.** The slice 12 → 18 sections (TRUST L1 signals, encounter row, poll questions, VOLUME bar, SUBSCRIBE footer) stay in the same order regardless of pip state. The data layer is sound; what was missing was _framing_, not architecture. Reordering would over-engineer a slice that's about copy + tone.

**Encounter / poll button palettes unchanged.** YES is solid black, NO is solid crimson. Keying the buttons to the pip palette would muddy the polling signal — the user is registering an opinion _about_ the writer, not echoing the existing pip colour. The crimson NO + black YES grammar is one of the panel's load-bearing decisions and stays untouched.
...
> **Superseded by Slice 37 (2026-06-02).** The standalone bell disc is gone; this panel now folds into the ∀ drop-up menu (`NotificationsAnchor.tsx` → controlled `NotificationsPanel.tsx`) and the unread badge moved onto the ∀. The data-plumbing description below still holds.

The Deferred list's _Notifications anchor (corner pip vs ∀-menu adjunct vs vessel)_ item retires. Slices 11–20 closed the active loops (reply, save, vouch, poll, encounter) for the _actor_ but the _recipient_ of those gestures had no surface on the chrome-less workspace floor — the Phase A topbar's notification badge retired with slice 1.5, and `/notifications` is a carry-over page the workspace user never has reason to visit. Slice 21 ships the anchor without rebuilding any data plumbing: the existing `notifications` table + `GET /api/v1/notifications` + `GET /api/v1/unread-counts` + `POST /notifications/:id/read` + `POST /notifications/read-all` (all already in `gateway/src/routes/notifications.ts`) are sufficient.
...
> **Superseded by Slice 37 (2026-06-02).** The standalone search disc is gone; this panel now folds into the ∀ drop-up menu (`SearchAnchor.tsx` → controlled `SearchPanel.tsx`). The data-plumbing description below still holds.

The Deferred list's _Search entry point_ item retires. Slice 21 closed the recipient/notifications gap on the chrome-less workspace floor; slice 22 closes the discovery gap. The pre-existing `gateway/src/routes/search.ts` (pg_trgm trigram index on `articles.title`, ILIKE on `accounts.username` / `display_name`, ILIKE on `publications.name` / `tagline`) is sufficient — no schema changes, no new endpoints, no backend work.
...
**Min query length.** 2 characters. Mirrors the gateway's `query.length < 2` 400-error guard so the client never fires a request that would bounce. Below the threshold, the component renders the _Type at least 2 characters_ hint instead of the loading spinner. The threshold is also why this slice doesn't bother with a "no results" pre-fetch state for empty input — _idle_ and _empty_ are visually distinct.
...
**Empty state.** Italic Literata 13px _No results for "${query}"._ The mono-caps "Searching…" loading state and the hint state share the same colour (`#9C9A94`) so the panel doesn't strobe between greyscale tones on each query.

**Footer = none.** Unlike the bell's _Open full log ›_ link to `/notifications`, the search popover has no footer pointing at the carry-over `/search` page. The workspace experiment is about retiring carry-over surfaces (per ADR §1: "Phase A topbar and the global ComposeOverlay-as-currently-shipped are retired on this branch"), and entrenching `/search` as a "see more results" destination would push against that. The popover's per-section limits (5/8/5) are the cap; deeper search is a future slice — either deepening the popover with pagination or, if usage justifies it, a search-vessel.
...
### Slice 23b — article cover images (2026-05-02)

The slice 23 deferral retires. Articles now carry an explicit cover image (NIP-23 `image` tag) authored from both the workspace `Composer` article mode and the legacy `/write` `ArticleEditor`, persisted on `articles.cover_image_url`, mirrored into `feed_items.media` so the workspace `MediaBlock` renders article hero images without translation, round-tripped through `article_drafts` so a scheduled draft keeps its cover, and surfaced on the reader hero with the legacy first-inline-image scrape preserved as a back-compat fallback for pre-23b articles.

**Schema (migration 081).** `ALTER TABLE articles ADD COLUMN cover_image_url TEXT` plus the same on `article_drafts`. Nullable (most existing articles have no explicit cover). No new index — covers aren't queryable; they're rendered alongside their owning row.

**Gateway publish path (`gateway/src/routes/articles/publish.ts`).** `IndexArticleSchema` adds `coverImageUrl: z.string().url().nullable().optional()`. The `INSERT INTO articles` writes the column; `ON CONFLICT … DO UPDATE` includes `cover_image_url = EXCLUDED.cover_image_url` so an edit to remove a cover (`coverImageUrl: null`) actually clears it. The `feed_items` dual-write composes a `[{type:'image', url}]` array when the cover is set and writes `null` otherwise; the `ON CONFLICT` upsert sets `media = EXCLUDED.media`. `GET /articles/:dTag` and `GET /articles/by-event/:nostrEventId` both include `coverImageUrl` in the response.

**Publication path (`gateway/src/services/publication-publisher.ts`).** `PublishToPublicationInput` gains `coverImageUrl?: string | null`. The submit-only branch (member without `can_publish`) writes the column on the `articles` row that's still pre-publish. The full-publish branch builds the NIP-23 event with an `image` tag (`if (input.coverImageUrl) tags.push(['image', input.coverImageUrl])`) before signing, threads the column into the `articles` INSERT/UPDATE, and dual-writes `feed_items.media`. `approveAndPublishArticle` (the editor-approves-submission path) now reads `cover_image_url` from the existing article row, emits the `image` tag against that, and dual-writes `feed_items.media` from the same value — so a cover authored at submit time survives editorial review without the editor re-supplying it.

**CMS PATCH (`gateway/src/routes/publications/cms.ts`).** `SubmitArticleSchema` and `EditArticleSchema` both gain `coverImageUrl`; the field map in PATCH adds `coverImageUrl → cover_image_url`. When the PATCH changes the cover, a parallel `UPDATE feed_items SET media = …` mirrors the cover into the unified table so the workspace surface reflects the change without waiting for a re-publish.

**Scheduler (`gateway/src/workers/scheduler.ts`).** `ScheduledDraft` interface adds `cover_image_url: string | null`; the `SELECT` from `article_drafts` pulls it. `publishPersonalDraft` pushes `['image', draft.cover_image_url]` into `baseTags` before signing v1, threads the column into both the `articles` upsert and the `feed_items` dual-write, and (paywalled drafts) v1's image tag survives onto v2 because v2 reuses `baseTags` unchanged. `publishPublicationDraft` forwards `coverImageUrl: draft.cover_image_url` into the `publishToPublication` call so the publication path picks it up.

**Drafts route (`gateway/src/routes/drafts.ts`).** `SaveDraftSchema` adds `coverImageUrl`; the upsert paths (dTag-keyed + new-draft + existing-no-dTag-update) all thread it. `GET /drafts/:id` returns `coverImageUrl` so `/write?draft=…` can hydrate the picker. The COALESCE pattern preserves the previous cover when an autosave omits the field — consistent with how every other optional field on the draft is handled. Explicit removal travels through the publish-time `coverImageUrl: null` payload, which writes NULL to `articles.cover_image_url` (the canonical column), not by clearing the draft.

**Web publish helpers (`web/src/lib/publish.ts`).** `buildNip23Event` emits `['image', data.coverImageUrl]` after the summary tag. The two `articlesApi.index({…})` calls (v1 free + v2 re-index after vault encryption) both include `coverImageUrl: data.coverImageUrl ?? null` so the publish-then-vault flow doesn't lose the cover on the second index. `publishToPublication` forwards `coverImageUrl` into `publicationsApi.submitArticle`. The two API client types (`articles.index`, `publications.submitArticle`) both gain the optional field. `ArticleMetadata` (`web/src/lib/api/articles.ts`) adds `coverImageUrl: string | null` so reader-page consumers see the explicit field.

**`PublishData` + `DraftData` (`web/src/components/editor/ArticleEditor.tsx`, `web/src/lib/drafts.ts`).** Both interfaces gain `coverImageUrl?: string | null`. The single `createAutoSaver` instance in each editor reads from a `coverImageUrlRef` (kept in sync with state via the `xxxRef.current = xxx` pattern already used for title/dek/price) so the closure always autosaves the latest cover.

**Cover picker — `/write` (`web/src/components/editor/ArticleEditor.tsx`).** New `coverImageUrl` + `coverUploading` state. New `handleCoverUpload(file)` calls the existing `uploadImage` helper (Blossom upload via gateway) and sets the resulting URL. Cover card sits between the standfirst card and the toolbar, with the same `bg-grey-100 px-5 py-4` chassis used for title/standfirst. Two states: empty → `+ Add cover image` button; present → 16:9 preview at 128–160px width plus `Replace` / `Remove` text-link buttons (using `.btn-text` / `.btn-text-danger` per the design tokens). `Remove` clears `coverImageUrl` to `null` (which round-trips as an explicit removal at publish-time). Threaded into the `PublishData` constructions in both `handlePublish` and `handleScheduleSubmit`. `/write?edit=` and `/write?draft=` both seed `initialCoverImageUrl` from the loaded data, and the `saveDraft` call in `handleSchedule` includes `coverImageUrl` so the scheduled draft round-trips.

**Cover picker — workspace Composer (`web/src/components/workspace/Composer.tsx`).** Same shape, fitted to the panel grammar. New state + ref + `handleCoverUpload`. New `pickCover` helper inside `ArticleModePanel` materialises the file picker. Cover slot sits between the standfirst input and the publish-as selector, on `TOKENS.fieldBg`. Empty state: `+ Add cover image`. Present state: 120px-wide 16:9 preview + `Replace` + `Remove`. `setCoverImageUrl(null)` reset on every Composer open. `coverImageUrl` flows into the `data` object in `handlePublishArticle` so both `publishToPublication` and `publishArticle` see it.

**Workspace card render (`web/src/lib/ndk.ts`, `web/src/components/workspace/WorkspaceView.tsx`, `web/src/components/workspace/VesselCard.tsx`).** `ArticleEvent` adds `media?: MediaItem[]` shaped exactly like `ExternalFeedItem.media` (so the existing `MediaBlock` consumes it without translation). `mapApiItem` forwards `item.media`. `ArticleVesselCard`'s standard + full density path inserts `<MediaBlock items={article.media ?? []} ctx={ctx} />` between the standfirst paragraph and the source-attribution row — same placement as `NoteVesselCard`. `MediaBlock` already early-returns `null` when items are empty, so an article without a cover renders as it did pre-23b. Compact density still suppresses media (the early-return in `MediaBlock` honours `ctx.density === 'compact'` from slice 23).

**Server-side feed shape (`gateway/src/routes/feeds.ts`, `gateway/src/routes/timeline.ts`).** Both `rowToItem` (workspace feeds) and `feedItemToResponse` (legacy timeline) include `media: row.media ?? null` on the article branch. `FEED_SELECT` already pulls `fi.media` (slice 23 wired this for externals); only the per-type response shape needed widening.

**Reader page hero (`web/src/app/article/[dTag]/page.tsx`, `web/src/components/article/ArticleReader.tsx`).** OpenGraph `image` prefers `article.coverImageUrl` over the legacy `extractFirstImage(contentFree)` scrape; the scrape is the explicit fallback, not a primary path. `ArticleReader` accepts `coverImageUrl` and uses it as `heroImage` directly — when present the body markdown is left intact (no `stripHeroImage` call), so an author can put an image at the top of their body without it being silently swallowed. Pre-23b articles (no `coverImageUrl`) keep the legacy scrape + strip path unchanged.

**Removal semantics.** Setting cover to `null` in either picker round-trips as `coverImageUrl: null` at publish time, the gateway INSERT writes NULL to `articles.cover_image_url`, the `image` tag is omitted from the NIP-23 event, and `feed_items.media` is set to NULL — so a cover-removed article reads identically to one that never had a cover. The `cover_image_url = EXCLUDED.cover_image_url` in the ON CONFLICT branch is what makes this real for edits; without it a removed cover would persist on re-publish.
...
**Fix (2026-05-09).** Resolver was failing to resolve most inputs. Five issues across three commits: (1) `@username` inputs (single `@`, no domain) fell through `classifyInput` to `free_text` because `PLATFORM_USERNAME` regex doesn't include `@` — the ILIKE search then included the literal `@` which matched no usernames; now `classifyInput` strips the `@` and re-tests. (2) `platform_username` did exact-match only so partial names returned nothing — now falls back to `searchPlatform()` ILIKE when exact match misses. (3) Phase B polling gave up after 3 seconds (3 polls × 1s) — external handles (Bluesky, Mastodon, RSS) routinely need longer; increased to 8 polls. (4) VesselBar silently swallowed all resolver errors (including 401 auth failures) — now surfaces "Resolution failed" in the dropdown. Same polling fix applied to `SubscribeInput`. (5) Root cause of all external resolution failures: the SSRF-hardened `buildPinnedAgent` in `shared/src/lib/http-client.ts` called the `connect.lookup` callback with `(address, family)` as separate args, but Node 20+ enables `autoSelectFamily` (Happy Eyeballs / RFC 6555) by default, which calls lookup with `{ all: true }` and expects the callback to return `[{address, family}]` as an array. The string address was indexed as an array, yielding `"Invalid IP address: undefined"` and silently killing every `safeFetch` call — all atproto, activitypub, RSS, and NIP-05 resolution. Same fix applied to `pinnedWebSocketOptions`. Resolver external-resolution logging upgraded from DEBUG to WARN so these failures are visible in production logs. Universal Feed ADR classification table and polling reference updated.
...
### Slice 25 — vessel collision detection (2026-05-10)

Vessels now push each other when dragged — the no-overlap rule from `WORKSPACE-DESIGN-SPEC.md` §"Feed scope" ("Feeds do not overlap. Overlap is the digital-workspace problem the physical metaphor shouldn't import") is enforced at drag time.

**New surface (`web/src/lib/workspace/collision.ts`).** Pure AABB collision resolver. `resolveCollisions(mover, others, floorBounds)` takes the dragged vessel's rect, all sibling rects, and the floor dimensions. Returns a `Map<feedId, {x, y}>` of position updates for pushed vessels. Uses BFS to handle cascading pushes (A pushes B into C, C gets pushed too), capped at 30 iterations. Floor-bounds clamping prevents vessels from being pushed off-screen. At floor edges where clamping can't resolve all overlaps, the system accepts residual overlap rather than fighting the user's drag.

**Store changes (`useWorkspace`).** New `batchUpdatePositions(updates)` action updates multiple vessel positions in a single state write (one React render, one localStorage debounce) rather than N separate updates per drag frame.

**Vessel changes (`Vessel.tsx`).** Three additions: (1) `data-vessel-id={feedId}` attribute on the `motion.div` so the floor can query vessel dimensions from the DOM. (2) `onDragStart` / `onDragFrame` callback props fire to `WorkspaceView` on drag start and every drag frame. (3) `isDraggingRef` guard + spring-animated position sync — when a vessel is _pushed_ (not being dragged), it slides to its new position with a `stiffness: 600, damping: 40, mass: 0.6` spring rather than teleporting. Respects `prefers-reduced-motion`. The spring config matches the existing `VESSEL_DRAG_TRANSITION` in `motion.ts`.
...
### Slice 37 — dock consolidation: one ∀, panels fold into the menu (2026-06-02)

Reverses the slices 21–22 cluster geometry. The bottom-right floor carried three discs left-to-right — ∀ (56px, dark) · bell (40px, white) · search (40px, white) — a "cluster grammar" that slices 21/22 justified as "∀ + subordinate workspace controls". Per the user's call this turn, that cluster collapses to a **single** control: the ∀.

- **Search + Notifications fold into the ∀ drop-up menu.** Two new menu rows below the create actions; selecting either opens the *existing rich panel* **in place** (anchored above the ∀, `bottom: 72`), not a navigation. A third new row, **Messages**, routes to `/messages` (DMs have no panel). Menu order is now: New feed · New note · Write an article — (gap) — Search · Messages · Notifications — (gap) — hidden-feed restores. Arrow/Home/End nav spans all rows.
- **The bell's badge moves onto the ∀, combined with DMs.** The ∀ now renders an unread badge = `dmCount + notificationCount`, sourced from the shared `useUnreadCounts` store (already polled every 15s by `AuthProvider`) — so there's one indicator for everything actionable. The Messages/Notifications menu rows also carry their individual unread chips (mirrors the topbar avatar menu).
- **Refactor.** `NotificationsAnchor.tsx` / `SearchAnchor.tsx` → renamed to controlled panel bodies `NotificationsPanel.tsx` / `SearchPanel.tsx` (panel dialog only — no own button, no fixed wrapper; the dock owns open/close via one shared outside-click + Esc handler covering button and whichever surface is open). `NotificationsPanel` **dropped its own 30s poll loop** (redundant with the shared store) and instead refreshes that store after mark-read / read-all so the ∀ badge stays in sync. `ForallMenu` became the single dock: a `view` state machine (`closed` / `menu` / `search` / `notifications`), a `forwardRef` `MenuRow` (React 18 — `ref` isn't a plain prop), combined-badge button. `WorkspaceView` drops the two extra mounts + imports.
...
### Slice 38 — feed composer: Glasshouse, no keylines, container-shape orientation, hoverable source names, per-source "no replies" (2026-06-05)

A polish + capability pass over `FeedComposer.tsx`, plus one schema column.

- **Glasshouse, not a bespoke scrim.** The composer dropped its own `position:fixed` scrim/panel (z-60, 1px black border, hand-rolled Escape) and now renders through the canonical `<Glasshouse>` primitive (frosted scrim z-55, white pane z-56, 6px slab top, ✕, Escape + body scroll-lock). It joins the reader/messages family as a frosted workspace overlay and correctly sits **below** the ForallMenu (z-60), which stays crisp. The pane scrolls (`my-8`, `overflow-y-auto`) rather than recentring, which removes a class of "modal jumps when it grows" jitter on its own.
- **No keylines anywhere.** Every 1px form is gone per the sitewide no-hairline rule: the panel border (Glasshouse owns enclosure), both text inputs (now recessed `#E6E5E0` fills, no border), the resolver match-row `borderTop`s (now whitespace + a hover wash), the Appearance and Delete section dividers (removed; the mono-caps section labels carry the structure), and the sampling chips' 1px borders (now fills). `scripts/check-hairlines.sh` clean.
- **Orientation icon depicts the vessel.** The glyph was a bare `|` / `─`. It is now an `OrientationGlyph` SVG (2px stroke, fixed 16×16 viewBox), so the button shows the *shape of the feed container* it sets. As of 2026-06-07 it draws the container as an **open `⊔` vessel**, open on the side it grows from — a tall portrait U open at the top for `vertical`, the same U on its side open to the right for `horizontal` — matching the `⊔`/`⊏` description of the orientations above (was a closed portrait/landscape rectangle). The fixed `104×34` `AppearanceControl` footprint from slice 36 is retained, so cycling still never reflows.
- **Source names route + hover like a card byline.** Each source label is now a `Link` to the same surface a feed-card byline targets — account → `/:username`, publication → `/pub/:slug`, external → `/source/:id`, tag → `/tag/:name` — via a new server-computed `display.href` on the source response (`sourceRowToResponse`). Account sources additionally get the full byline hover (`useAuthorHover` + `AuthorModal`, `type="author"` keyed on `accountId`); `AuthorModal` gained an optional `zIndex` prop so the portalled hover card (default z-50) floats above the Glasshouse pane (passed z-70).
- **Per-source "no replies" mode (migration 107).** New `feed_sources.exclude_replies boolean DEFAULT false`. A `NO REPLIES` chip sits beside volume and the random/top pair in each `SourceRow`; `PATCH …/sources/:sourceId` accepts `excludeReplies`. `sourceFilteredItems` computes `allow_replies = bool_or(NOT fs.exclude_replies)` in its `matched` CTE and drops reply items (`fi.is_reply`, migration 097) unless at least one matching source still admits replies. Native note replies remain globally excluded regardless (the pre-existing `n.reply_to_event_id IS NULL` clause), so in practice this flag is what suppresses **external** (Bluesky/Mastodon/…) replies for a source — "you only get freestanding announcements."
- **Steadiness (the shudder).** The remaining jerk was the mute toggle showing/hiding the sampling controls, reflowing the row. The sampling + no-replies controls now **always render** (dimmed + disabled when muted) so muting never changes the row's height, and every per-source commit is **optimistic** (instant local update, reconcile on success, revert on failure) with the feed behind the glass reloading **once** after commit (the `SourceRow` `onChanged`/`onCommitted` split) rather than on every tick.
...
### Slice 40 — ∀ constructed as ground-coloured bars inside the disc (2026-06-06)

Retires the typed glyph entirely. Both Slice 39 (aperture crop) and the first cut of this slice (a shrunk, fully-enclosed `∀` character) leaned on the font's `∀` (U+2200), which never seated cleanly in the disc — the crop left rim slivers reading as a fine outline, and the enclosed character looked low/uncentred regardless of nudging. The mark is now **constructed geometry**, not a letter: three bars forming the A skeleton, drawn in the **workspace floor colour** so they read as the ground cutting through the black disc, with the diagonals reaching the rim to carve complete circle segments.

- **The figure.** Two symmetric diagonals run from a shared point at the bottom of the rim up to the rim on each side — each cutting off a **complete circle segment** — joined by a horizontal **crossbar** across the central region. The A/∀ skeleton, thought of as bars dividing the disc rather than a letterform. Bold (`strokeWidth: 5`, round caps), not hairlines.
- **Reaches the rim, *not* clipped (kills the hairline).** The glyph span is replaced by an inner `<svg viewBox="0 0 56 56">` (`position:absolute; inset:0`). The bars are deliberately **not** clipped to the disc. An earlier cut clipped them at `r=28`, which left a dark hairline tracing the disc outline at the three points where the bars meet the ground: the clip stopped the floor-coloured stroke exactly at the rim, so it never covered the disc's own anti-aliased edge pixels, and that black AA edge showed through between the floor bar and the floor ground. Removing the clip lets the diagonals' upper ends run just past the circumference (their endpoints sit at `r≈30`) and the apex's round cap spill just past the bottom; painted on top of the disc, that overshoot covers the edge AA at each contact, so no hairline shows. The overshoot is invisible (floor-on-floor) and the visible bar still ends at the rim (the black disc ends there), so the segments still read as complete; the **black segments** the bars carve out are what mark the disc against the ground. The SVG is a sibling of the unread badge. Coordinates: left diagonal `(28,56)→(8.5,5)`, right diagonal `(28,56)→(47.5,5)` (upper endpoints intentionally just outside the disc), crossbar `(18.1,30)→(37.9,30)` (endpoints computed to land on the diagonals at `y=30`).
- **Colour = floor.** `stroke={TOKENS.glyphFg}` is now `#F0EFEB` — the workspace `FLOOR` value in `WorkspaceView.tsx` (also the disc's old `buttonFg` cream). The button's `font-serif` class is dropped (no text to set a face on).
- **No disc outline.** The button's `boxShadow` (the soft elevation ring that read as an outline) is removed; the black disc still reads clearly against the cream ground by contrast alone.
- **Tuning knobs.** Diagonal upper endpoints (`8.5,5` / `47.5,5`, kept just outside the disc) set the splay; the shared `(28,56)` is the bottom point; crossbar `y` (30) and its leg-intersection x's (`18.1`/`37.9`) set its height; `strokeWidth` sets bar weight.
- **Site favicon derives from this mark.** `web/public/favicon.svg` is the same construction at the same coordinates — black disc, floor-coloured (`#F0EFEB`) ∀ bars — but the bars *are* clipped to the disc (a browser tab has no workspace floor behind it, so there's no floor-on-floor seam to avoid; clipping keeps the tab corners clean/transparent). PNG fallbacks rasterised from it with `sharp`: `icon-32.png` (32×32, transparent corners) and `apple-touch-icon.png` (180×180, flattened on the disc black so iOS's transparency-flatten + squircle-crop yields a clean black icon with the cream ∀). All three are wired in `web/src/app/layout.tsx`. Caveat: the disc is near-black, so on a dark browser-tab theme the disc edge blends into the tab and the mark reads as the cream ∀ alone (still recognisable); a `prefers-color-scheme` variant could be added if that matters.
...
- **Rotate-on-hover, hold a right-side-up A.** While the cursor (the house) hovers the button, the ∀ glyph rotates to **180° and holds** — i.e. it settles as a right-side-up **A** for as long as you hover. Moving away **completes** the turn to 360° (back to ∀), then snaps invisibly to 0° ready for the next hover. Driven by a `glyphRot` state + a `transform: rotate()` transition on the inner `<svg>` (`480ms ease-in-out`), not a one-shot keyframe animation: `onMouseEnter`→180, `onMouseLeave`→360, and `onTransitionEnd` resets 360→0 with `transition: none` (a `spinTransition` flag) so the reset doesn't visibly unwind. Only the glyph SVG turns — the unread badge is a sibling and stays put. (The earlier cut spun a single 360° on each mouse-enter via `@keyframes forall-spin`; that's replaced by this hover-hold behaviour.)
...
- Volume TOP metric definition (slice 14 records the value; slice 16 maps it to `feed_items.score × weight`; refining what TOP _means_ beyond reusing the existing score remains open).
