...
| §1.3 | ∀ workspace control + four-item menu | `web/src/components/workspace/ForallMenu.tsx` (slice 2: button + menu shell). All four item handlers now wired: *new note* → `Composer` (slice 2.5), *new feed* → `NewFeedPrompt` (slice 3), *fork feed by URL* → `ForkFeedPrompt` (slice 8), *reset workspace layout* → `ResetLayoutConfirm` (slice 6). |
