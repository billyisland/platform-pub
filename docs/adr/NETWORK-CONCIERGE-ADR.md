...
- **Phase 2 — ASSISTED atproto.** "Set one up for me" → Bluesky via OAuth account-creation-in-flow (§6.1). **Reuses the LINKED path** — no PDS, no `did:plc`, **no §8.1 gates**. Behind a flag. **This is the new default satellite path.**
