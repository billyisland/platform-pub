...
**Deploy ordering is the REVERSE of the usual, and of migration 177's.** Old code names the dropped column, so it is deploy-then-migrate: new code against an unmigrated DB is fine; old code against a migrated one 500s on merge, delete and the admin seed panel. The window was measured rather than asserted, and it is narrower than the first draft of this paragraph claimed: reading and writing feeds is untouched, and so is **seeding** — old `seedStarterFeeds` names the column only in its fallback arm, unreachable while a formula is designated (the precondition of the migration existing), and `listFeedsForOwner` catches the whole call anyway. Two destructive routes and one admin panel, not the site. The migration header carries the same correction, because a deploy note that overstates its blast radius is how deploy notes stop being read.
