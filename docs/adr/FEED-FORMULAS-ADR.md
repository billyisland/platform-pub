...
- **Phase 2 — absorb the starter template. STEP 1 SHIPPED 2026-08-12 (§14).** Cut the live template into a designated default-seed formula, repoint `seedStarterFeeds`, verify seeding end-to-end on a fresh account, *then* drop `is_starter_template` and its two guards in a following migration. Never in one step: the flag is the only thing keeping seeding alive until the formula replaces it. Step 1 (the code + the designation surface, with the flag still standing) is done; step 2 is the drop, and its precondition is a designated formula on **prod**.
...
**What is NOT done, and its gate.** Dropping `feeds.is_starter_template` with the merge and delete guards, the clone path, and the legacy provenance arm. The gate is a designated formula **on prod** — until then the fallback is what seeds real signups.

---

**Phase 2 step 2 is the next step and its ordering is load-bearing:** only after a formula is designated on prod and a real signup has been seeded from it, drop `is_starter_template`, its two 409 guards, `cloneFeedForOwner`, and `seedStarterFeeds`'s fallback arm. Never before. And the seed path must not be gated on `FEED_FORMULAS_ENABLED` (§6).
