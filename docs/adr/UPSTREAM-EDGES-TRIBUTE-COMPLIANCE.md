...
5. **Reversal path (F3) is a hard build gate, not a sign-off item — UNBUILT (2026-06-24).** Unlike #1–#4 (which the compliance owner *confirms*), this is an engineering prerequisite the flag flip waits on. The ADR requires a reversed read to unwind the author's credit **and** its tribute accruals; no such handler exists, and the post-build audit (`UPSTREAM-EDGES-AUDIT-FIXES.md` F3) found **no reader-chargeback/refund path anywhere on the platform** (`payment-service` webhook handles only payment-intent / transfer / account events — no `charge.dispute.created` or refund). Consequence if the flag flips before this ships: a reader chargeback *after* settlement leaves the tribute accruals — and, once paid, the real Stripe Connect transfers to the inspirer — **un-reversed**. The fix is platform-wide first (a `charge.dispute.created`/`charge.refunded` handler that rolls a settled read back), then the tribute-subtree void composes onto it (`held|released` voided in place; `paid` posts a reversing `tribute_payout`). **`TRIBUTES_ENABLED` MUST NOT be set in production until F3 ships.** Tracked in the build plan's Phase-3 *as built*.
...
Keep the **accrue-from-creation** model (no design change to the ADR), **enable Phase 3 only after** (a) the compliance owner confirms baseline-posture point #1 and characterisation point #2, ~~(b) the Phase 2 contact/render copy is audited and corrected to condition #2's "conditional offer, not held-in-your-name" wording~~ **(b) — done 2026-06-23** (see condition #2 above), and **(c) the F3 reversal path (residual #5) ships** — the one remaining *engineering* gate, not a sign-off. So two gates remain: the human sign-off (a) and the F3 build (c). Dial A is the fallback if #2 cannot be confirmed; Dial B is parked as a baseline-float option, not a tribute mechanism.
