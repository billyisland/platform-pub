...
5. **Reversal path (F3) is a hard build gate, not a sign-off item — ✅ BUILT (2026-06-24), gate cleared.** Unlike #1–#4 (which the compliance owner *confirms*), this was an engineering prerequisite the flag flip waited on. The ADR requires a reversed read to unwind the author's credit **and** its tribute accruals; there was previously **no reader-chargeback/refund path anywhere on the platform**. Now built (migrations 132–133, `settlement.reverseSettlement` + the pure `chargeback.ts` planner, webhook `charge.dispute.closed` [lost] + `charge.refunded` [full]): platform-wide reversal first — a reversed charge restores the reader's debt + reverses the writers' already-paid net and rolls its reads to `charged_back` — with the tribute-subtree void composed on top (`held|released|swept` unpaid → voided; `paid`/`returned` → reversing `tribute_payout_reversal`/`writer_payout_reversal`, telescoping to −read_net, proven by 8 conservation tests). Owner-confirmed decisions: reverse on dispute-**lost** + **refund**; **restore the reader's debt**; **include upvotes**. Tracked in the build plan's Phase-3 *as built* + `UPSTREAM-EDGES-AUDIT-FIXES.md` F3. **So this no longer gates `TRIBUTES_ENABLED`** — only residual #1 (the human sign-off) remains.
...
Keep the **accrue-from-creation** model (no design change to the ADR), **enable Phase 3 only after** (a) the compliance owner confirms baseline-posture point #1 and characterisation point #2, ~~(b) the Phase 2 contact/render copy is audited and corrected to condition #2's "conditional offer, not held-in-your-name" wording~~ **(b) — done 2026-06-23** (see condition #2 above), and ~~(c) the F3 reversal path (residual #5) ships~~ **(c) — done 2026-06-24** (residual #5 built; see above). **So one gate remains: the human sign-off (a).** Dial A is the fallback if #2 cannot be confirmed; Dial B is parked as a baseline-float option, not a tribute mechanism.
