...
**Status:** Accepted in design discussion (2026-07-12). This document records the model, the per-source scoping, and the phasing. Feasibility survey done against source 2026-07-12 (file refs below verified at that date). **Scoped for implementation 2026-07-12** (§11): every §2 claim was re-verified against source; §§2–9 carry same-date *[Scoping amendment]* notes where the code sharpened or corrected the design, and §11 holds the concrete build plan (prerequisite refactors, migration contents, per-phase file lists). **Phase 0 + the 1a/1b backend BUILT 2026-07-12** (dark behind `FOLLOW_IMPORT_ENABLED`): the §11.1 refactors, migration 153, the engine + sweep + `POST/GET /follow-imports` routes, both §6.4 stampede fixes, and the atproto/Nostr graph readers — FIX-PROGRAMME 2026-07-12. **Phase 1a/1b web surfaces BUILT 2026-07-12** (same flag, second entry in FIX-PROGRAMME same date): the three §7 surfaces (post-link offer riding `&follows=` on the Bluesky callback redirect → `PostLinkImportOffer`; NetworkReachPanel per-presence "Import follows" + the paste-an-identity `FollowImportSection`; FeedComposer "…or import everyone they follow"), the `followImportProtocols` capabilities gate, `useFollowImportRun` + `FollowImportStatus` progress polling, and live-workspace feed adoption (`useFeedArrivals`). The §10 invariants are now in CLAUDE.md. **Phase 1d OPML BUILT 2026-07-12** (same flag, third entry in FIX-PROGRAMME same date): jsdom-based parser/planner (`gateway/src/lib/opml.ts` — folder→feed mapping, `OPML_MAX_FEEDS` 10 with overflow folded into the base feed, invalid entries counted, 1000-cap in plan order), `POST /follow-imports/opml` (one run per planned feed, **no sync binding** per §5.4), the engine's rss probe-on policy (D6 exception — dead entries → `failed`), the `followImportOpml` capability, and the `FollowImportSection` upload block (preview/confirm → multi-run progress via `useOpmlImport`). **Phase 2 "Sync now" BUILT 2026-07-12** (same flag, fourth entry in FIX-PROGRAMME same date): migration 154 (`kind`/`removals`/`removal_cursor`/`removed` + `preview` status on `follow_imports`), `POST /follow-imports/sync` (diff persisted as a preview run; zero diff stamps `last_synced_at`), confirm/cancel endpoints, the engine's removals-before-adds sync path (`removeSource` with `recordExclusion:false` — a sync removal mirrors a remote unfollow, never an exclusion; apply-time exclusion re-check on adds), the truncation guard (a capped graph read suppresses removals, surfaced as `removalsSkipped` — §11.5 amendment below), the `addSource` exclusion-revoke on manual re-add (§6.3 symmetry), and the FeedComposer `FeedSyncSection` (binding rides `GET /feeds/:id/sources`). **Phase 1c ActivityPub BUILT 2026-07-12** (fifth entry in FIX-PROGRAMME same date, behind the §6.6 sub-brake `FOLLOW_IMPORT_ACTIVITYPUB_ENABLED`, default off pending the §6.4 soak): the §2 live scope check PASSED first (`read:accounts` authorises the following endpoint per mastodon/mastodon source — **no re-consent flow needed**; residual: one authed self-import against a real token before the prod flip), the Mastodon-API reader (§11.4 note), the Mastodon post-link offer, and the `unresolved`-suppresses-sync-removals guard (§11.5 note 2 extended). **Dev-stack run-through PASSED 2026-07-12** (sixth entry in FIX-PROGRAMME same date): all four protocols exercised live (atproto 11/11 + 618/618 with the §6.5 volume default and D6 metadata verified; Nostr 251/251 bare-hex; AP public leg 717/717 `unresolved: 0`; OPML folder→feed with the dead entry surfaced in `failed`), plus the full Phase 2 loop (preview → confirm removal; sync removals append no exclusion; a local exclusion suppresses the re-add; manual re-add revokes it) and the §6.4 rails at ~1,600 sources. One bug found and FIXED: all three readers derived `truncated` from the remote's *claimed* count (atproto `followsCount` / Mastodon `following_count` include deactivated accounts the list endpoints omit), so nearly every aged account read as permanently truncated — which made §11.5's guard suppress sync removals forever. `truncated` now comes only from the pagination machinery (`getFollows`/`fetchMastodonFollowing` return `{…, complete}`; claimed counts stay display-only in `total`). The two flags are now wired into `docker-compose.yml` (`:-0` defaults). **Phase 3 "Bring your world" BUILT 2026-07-12** (eighth entry in FIX-PROGRAMME same date — the seventh is the run-through's graphile_worker schema.sql fix): the `import` resolver context (external-first, squatter-proof — §7.4 build note) and the first-session `BringYourWorld` Glasshouse sheet riding the founder-feed-mint signal (no signup wizard exists; the sheet folds into CONSOLIDATED-TODO §3.3's wizard when that builds). Remaining: flip the master flag in prod (deploy-time env); flip the AP sub-brake after the §6.4 soak + the authed live check; a browser-level look at the sheet on next dev browse (API + build verified; the render path itself wasn't eyeballed).
...
| Read Mastodon follows | ✅ **verified at 1c build (2026-07-12)** | `MASTODON_SCOPES = "read:accounts write:statuses"` (`gateway/src/routes/linked-accounts.ts:39`); mastodon/mastodon main's `following_accounts_controller.rb` authorises `:read, :'read:accounts'` → the granted scope covers `GET /api/v1/accounts/:id/following` — **no re-consent flow needed**. Bonus from the source: `hide_results?` is bypassed for the authed self-call, so a linked token reads hidden follows too. Live against mastodon.social: endpoint + `lookup` public; Account entities carry the actor `uri` (canonicalisation free on ≥4.2); Link-header `max_id` pagination, newest first; hidden follows = empty list. Residual: the authed leg untested against a real token (no linked AP presence in dev) — one authed self-import before flipping `FOLLOW_IMPORT_ACTIVITYPUB_ENABLED` in prod |
...
`FOLLOW_IMPORT_ENABLED` operator master switch, dark-shippable per house pattern. Per-protocol sub-flags only if a specific protocol needs an independent brake (likely `activitypub`, pending the fairness work proving out). *[1c build 2026-07-12: the anticipated brake exists — `FOLLOW_IMPORT_ACTIVITYPUB_ENABLED`, ANDed with the master switch, default off; flip it only after the §6.4 soak and one authed live self-import (amended §2 row).]*
...
4. **Onboarding — "Bring your world"** (Phase 3): a signup-flow step presenting the omnivorous input (paste any handle / npub / @user@instance / feed URL, or upload OPML) → resolver classifies (new `import` context for `POST /api/resolve`) → offers the feed. Lands alongside the writer-onboarding wizard work (CONSOLIDATED-TODO §3.3) — the reader-side half of the same first-session problem. *[Built 2026-07-12 — two decisions the build settled:]* (1) **there is no wizard to land alongside** (§3.3 is unbuilt; there is no signup flow beyond `/auth` → `/reader`), so the step rides the one first-session signal the product has: the workspace bootstrap that mints the founder feed (zero feeds = brand-new account, the same discriminator the parked first-login ceremony uses). `BringYourWorld` (`web/src/components/workspace/BringYourWorld.tsx`, lazy-chunked) is a Glasshouse sheet wrapping the §7.2 `FollowImportSection` (paste + OPML — the whole omnivorous surface was already parent-agnostic), shown once per user (localStorage seen-key written only on actual dismissal, so a dark `FOLLOW_IMPORT_ENABLED` — which renders no sheet at all via the capabilities gate — never burns the one shot), suppressed when a deep-linked overlay already claimed the Glasshouse, D7-strict (pure offer; the evergreen paths in Network/FeedComposer are named in its closing copy). When the §3.3 wizard is eventually built, this component folds in as its reader-side step. *[2026-08-07 — done: it folded in exactly as written. `BringYourWorld.tsx` is DELETED and its offer is step 2 of `web/src/components/workspace/Welcome.tsx`, unchanged in substance — same `FollowImportSection`, same `import` context, same D7 purity, and still ABSENT entirely (not empty) when the capabilities gate reports nothing importable, so a dark `FOLLOW_IMPORT_ENABLED` yields a three-step sheet rather than a hollow one. The one thing that changed is the trigger, and it had to: the zero-feeds signal this note describes as "the same discriminator the parked first-login ceremony uses" had silently stopped meaning "brand-new account" — `/workspace/bootstrap` seeds starter feeds before returning, so zero feeds is the seeding-FAILURE path, and the sheet was reachable on prod only because the §0l incident had destroyed the template. The gate is now `accounts.onboarded_at` (migration 176), a fact about the member rather than about their feeds. See CONSOLIDATED-TODO §3.3.]* (2) The **`import` resolver context** shipped as designed: external-first ranking (`contextPriority`), and — the piece `subscribe` pioneered in the 2026-07-10 squatter amendment — an exact native-username hit does not short-circuit the external world (import consumes only external sources, so the squatter case matters *more* here); `FollowImportSection` now passes it. Verified live: `import` ≡ `subscribe` on squatted + unsquatted queries, ≠ `general`/`dm`.
...
- **Phase 3 — onboarding integration**: the "Bring your world" signup step + `import` resolver context; coordinate with the onboarding-wizard build (CONSOLIDATED-TODO §3.3). *[Built 2026-07-12 as a first-session workspace sheet — no wizard exists to coordinate with; see the §7.4 build note.]*
...
- **1c ActivityPub (M, gated on §6.4 having soaked under 1a/1b)**: **live scope check first** (§2 amendment — re-consent risk); following-list reader in `gateway/src/lib/activitypub-resolve.ts` (bearer pattern from `fetchMastodonProfile`, Link-header pagination, per-account WebFinger canonicalisation with per-host throttle, hidden-follows detection via empty-list + `following_count` > 0). *[Built 2026-07-12 — five facts the build settled:]* (1) the scope check passed (amended §2 row) so no re-consent flow was built; (2) canonicalisation is nearly free — the REST Account entity's `uri` IS the actor URI for local and remote entries alike (≥4.2 origin instances), so the per-account WebFinger fallback only runs for pre-4.2 origins, per-host-throttled (4 host-groups parallel, sequential within a host), and unresolvable entries are dropped-but-counted (`unresolved` on the graph result → create response → status line); (3) the reader ships behind the §6.6 per-protocol brake `FOLLOW_IMPORT_ACTIVITYPUB_ENABLED` (default off) so the master flag can go live for 1a/1b/1d while AP waits out the soak — `importableProtocols()` lights the capability with no web change; (4) the API host is pinned by WebFinger (the actor URI's origin), not the acct domain — split-domain installs resolve correctly — and the Link-header pager only follows same-origin next URLs (the header is remote-controlled input); (5) a matching linked presence's token is used when the pasted/linked identity IS the requester's (`handle` match), with one public retry on token failure; the authed self-call bypasses `hide_results?` so a linked account imports its own hidden follows.
...
*[Built 2026-07-12 — four mechanics the build settled:]* (1) the preview is **persisted** (status `preview`, adds in `identities`, removals in a new `removals` column, both cursored) so confirm applies exactly the plan that was shown, restartably — not a re-read that may have drifted; (2) a **truncated** graph read suppresses removals entirely (past the cap "unfollowed" is indistinguishable from "outside the newest-N window" — removing on it would drop still-followed accounts), surfaced as `removalsSkipped` *(extended at the 1c build: an AP read with `unresolved > 0` suppresses removals the same way — a dropped-but-still-followed entry must not read as an unfollow)*; (3) sync removals call `removeSource` with `recordExclusion: false` — they mirror remote unfollows, not local intent, and recording them would block a future re-follow from syncing back in — while sync adds re-check exclusions at apply time so a deliberate removal made between preview and confirm wins; (4) the §6.3 exclusion gains its inverse: a **manual re-add** to a bound feed revokes the exclusion in `addSource` ("the user who wants it back re-adds it"), and `computeSyncDiff` leaves any residual excluded-but-member row untouched in both directions. Sync runs skip the §6.5 volume default (the feed's volume character is the user's by then). Diff logic is the pure `computeSyncDiff` in `follow-import.ts`; the surface is the FeedComposer's `FeedSyncSection`, gated on the binding riding `GET /feeds/:id/sources` **and** the protocol appearing in `followImportProtocols`.
