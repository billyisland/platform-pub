...
- **External (Bluesky/Mastodon) — parents-above (full chain).** `GET /external-items/:id/thread` already returns the full `ancestors`/`descendants`. `ExternalVesselCard` now fetches the thread once (`useExternalThread`) and renders the **full ancestor chain above the card content** via `ExternalAncestorRail` (replacing the single immediate-parent `ParentContextTile` in the expanded state), with descendants below via `ExternalCardThread` (now descendant-only, taking the shared fetch as props). **In-place re-focus for external is deferred**: the source `/thread` is item-relative + depth-limited so re-focus needs a refetch, and the rich host-card body (media/polls/quotes) doesn't map cleanly onto a lightweight focal entry — tracked as a follow-up. The collapsed-state single-parent `ParentContextTile` preview is unchanged.

Implemented in `gateway/src/routes/replies.ts`, `web/src/lib/api/feed.ts`, `web/src/hooks/useConversation.ts`, and `web/src/components/workspace/{ConversationView,ExternalAncestorRail,VesselCard}.tsx`.
