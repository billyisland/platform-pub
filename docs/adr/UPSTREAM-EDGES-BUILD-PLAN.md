...
- **Frontend — deferred (focused follow-up):** (1) the inline-in-prose citation marker anchored at `char_start` inside the `dangerouslySetInnerHTML` body — needs prose-DOM injection + runtime iteration; the apparatus renders the cited-author dispute at the foot meanwhile. (2) Authoring UI (create credit/citation, file/withdraw dispute) — endpoints are live, only the create forms are missing.
...
1. **Phase 1** — edges + dispute stake. ✅ **Shipped 2026-06-22** (backend + read-side rendering; inline-prose marker + authoring UI deferred — see *Phase 1 — as built* above). Established the ledger-guard wiring.
