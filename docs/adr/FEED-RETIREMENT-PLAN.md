...
**Status:** Effectively complete — Slices 1–7 shipped 2026-06-12; Slice 0 decided (option a). `/feed` is now a redirect shim; the front door lands on `/workspace`; reach (Following/Explore) is a composable source kind; new accounts seed a starter-feed clone; the four Bucket-B routes (`/network`, `/library`, `/subscriptions`, `/profile`) are now workspace overlays. Shared feed SQL is extracted to `gateway/src/lib/feed-sql.ts`; the legacy `GET /feed` reach handler (`timeline.ts`) and `GET /conversation/:eventId` are deleted. The legacy `components/feed/` card stack (`FeedView`/`ExternalCard`/`ArticleCard`/`NoteCard`/`QuoteCard`/`NeighbourhoodCard`/`ActionSheet`) + `useNeighbourhood` + the dead `FeedDial` control are deleted; `AuthorModal` (shared) stays, `SubscribeInput` relocated to `components/subscriptions/`. **Operator prereq before seeding does anything: flag ≥1 of your own feeds as a template** — `UPDATE feeds SET is_starter_template = true WHERE id = '<feed-uuid>';`. Only remaining: Slice 6 item 4 (workspace items-path convergence — tracked follow-on, non-blocking).
**Date:** 2026-06-12
