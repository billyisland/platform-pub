...
**§IV (the floating disc and its lens) is superseded-in-waiting, 2026-07-22, by `docs/adr/WORKSPACE-COLUMN-LAYOUT-ADR.md` §VI**: the ∀ leaves its floating position for a fixed opaque nav row along the bottom of the viewport (z-58), and the difference-blend lens is **deleted** with its satellites (`lensMode` and the stacking choreography in `ForallMenu.tsx`, `stores/lensSuppress.ts`, the `body { isolation }` scope, the canvas `isolation`) — the blend costs a viewport-sized compositing surface per scroll frame, and against a bounded taut floor with a fixed row there is nothing left for it to float over. **The mark itself, the punched construction, the menu and the ceremony are all retained**; only the blend and the float go, and only when that ADR's Slice 4 lands (Slice 0 shipped 2026-07-22; Slices 1–5 open). Until then §IV governs and the lens must not be dismantled piecemeal — its stacking rules are load-bearing while it exists.
