...
**Status:** Accepted; implemented 2026-07-21. Three related changes, all shipped: §III canonical cut mark (assets in `web/public/brand/`); §IV the idle trigger as a window onto the workspace (incl. the negative-lens iridescence, §IV.5 — **since deleted, see the note below**); §V lockup rebalance (disc 46 / wordmark 28, dropdown gap tracking `discSize` — the ratio survives at the nav row's 40 / 24). The §IV.3 composite/stacking questions were settled by the pre-implementation prototype (§IV.5 records its findings); the §VII in-browser eyeball pass over live feed schemes remains the standing verification.
**§IV (the floating disc and its lens) is SUPERSEDED — Slice 4 landed 2026-07-22 — by `docs/adr/WORKSPACE-COLUMN-LAYOUT-ADR.md` §VI**: the ∀ leaves its floating position for a fixed opaque nav row along the bottom of the viewport (z-58), and the difference-blend lens is **deleted** with its satellites (`lensMode` and the stacking choreography in `ForallMenu.tsx`, `stores/lensSuppress.ts`, the `body { isolation }` scope, the canvas `isolation`) — the blend costs a viewport-sized compositing surface per scroll frame, and against a bounded taut floor with a fixed row there is nothing left for it to float over. **The mark itself, the punched construction, the menu and the ceremony are all retained**; only the blend and the float went. **§IV is now history — read it for the reasoning, never as a rule to defend.** What survives of this ADR and still governs: §III's canonical cut mark (exports in `web/public/brand/`, the one ground that is ours to show — the workspace disc paints), and §V's lockup, whose disc-to-cap-height ratio is why the wordmark and disc stayed **adjacent** when the ∀ moved into the nav row rather than splitting to opposite ends of it (the pair is one fixed container at the row's right end; the row-anchored sizes are disc 40 / wordmark 24, the 46/28 ratio carried over so 40 + 2·GRID lands exactly on `NAV_ROW_H = 56`).
