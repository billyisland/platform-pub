...
- **Click on a discoverable target** pins that target's annotation: the pinned channel's `index` jumps to the matching annotation (creating one on the fly for hover-only instance targets). Clicks therefore have a job, and it is an Explain job, not a product job.
- **Click anywhere else** (not a bubble, not the About button, not a target) dismisses, per §7.
...
- **Explain:** Esc (subject to precedence, D12) or click on any non-target, non-bubble, non-About region. Completing the last sequential step is **not** an auto-dismiss (re-reading and hovering remain available). No toggle dismiss (D3).
- **First-run:** same gestures, plus an explicit "done" affordance on beat 6. One gesture kills it.
