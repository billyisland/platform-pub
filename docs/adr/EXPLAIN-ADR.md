...
1. **First-run auto-entry is dormant.** Landing in Explain mode on load without asking for it read as a malfunction, not a welcome. `FirstRunController` is no longer mounted (kept intact in `ExplainProvider.tsx` for revival); Explain is strictly ∀-menu-invoked (amends D6).
   ...
   *[2026-08-07 — the tour came back, the auto-entry did not.]* This amendment was a verdict on the ENTRY, not on the six beats, so the beats are now **offered** on the last step of the first-session welcome (CONSOLIDATED-TODO §3.3, `Welcome.tsx`) and opened by `FirstRunLauncher` on an explicit yes. `FirstRunController` remains dormant and un-mounted; nothing auto-enters. The launcher exists as a separate component purely to own the ORDERING — floor-mode Explain sits below the Glasshouse band, so it mounts only once the welcome pane has gone, or the accepted tour would render behind the sheet that offered it. D6's gate (c) "BringYourWorld not showing" is moot: that component is deleted, absorbed as the welcome's step 2.
