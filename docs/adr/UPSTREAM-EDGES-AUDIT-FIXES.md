...
**Status (2026-06-24):** P0 **F1/F2 done**; P1 **F4/F7/F10/F11 done**, **F3 deferred & documented** as a hard gate on the flag flip (compliance residual #5). P2 DB-backstop trio **F5/F6/F8 done** (migration 131 + the consent/decline/claim 23514 mapping). The remaining open work is the P2/P3 items with no DB surface (F9 by-design, F13 frontend robustness, F12/F14/reserved-copy nits) — none blocks the flag. The two flag gates are still (a) the compliance human sign-off and (c) building the F3 reversal path.
