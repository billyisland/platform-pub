...
- [x] **F3 · Refund/chargeback unwind is required by the ADR but unbuilt.** ✅ BUILT 2026-06-24 (clears flag-gate (c))
  ...
  **As built** (decisions confirmed with the owner: reverse on dispute-**lost** + **refund**, **restore the reader's debt**, **include upvotes**):
  ...
  - **Residual (documented, not a blocker):** an accrual a payout has *already reserved* but not yet paid at the instant a chargeback lands (the rare chargeback-during-payout-cycle window) is skipped — neither voided nor reversed — and left for that payout to resolve. Tributes are dark, so this is unreachable in production today.
