...
1. **Coalescing + backfill + self-heal run in the gateway scheduler, not as a feed-ingest graphile task.** The gateway already runs a 60s advisory-locked worker loop (`publishScheduledDrafts`); a second sweep (`runDiscoverySweep`, advisory lock `DISCOVERY`) was added there because that is where signing is possible. So the ADR's "deferred `republish_follow_list_<accountId>` graphile job, job_key-coalesced" (§3.4) is realised instead as an `accounts.follow_list_dirty` marker set on follow/unfollow + external-nostr sub/unsub, drained once per cycle by the sweep (rebuild-from-state → sign → enqueue → clear flag). This preserves both properties the ADR wanted from the job_key approach — burst coalescing and final-state correctness — without a cross-service signing path. **The dirty flag is cleared *before* the follow-set read (claim pattern), not after**, so a follow/unfollow that lands mid-republish re-marks the row and is caught next cycle rather than being clobbered by a trailing clear; failure restores the flag (audit D5, fixed 2026-06-07).

2. **One sweep does backfill *and* self-heal**, keyed on a new `accounts.discovery_synced_at` column (NULL = never published; re-published at least every 7 days). New accounts (NULL, sorted first) are picked up within a cycle, so **account-creation triggers were not wired** at `signup()`/`createGoogleAccount()` — the sweep covers them (≤60s latency, acceptable for discovery). This is the "self-heal first" sequencing the durability decision called for; the per-relay ACK worker change (§3.3 item 1, Risk #1) shipped 2026-06-07 — see item 7.

7. **Per-relay delivery accounting for discovery rows (§3.3 item 1 / Risk #1) — shipped 2026-06-07, simpler than the originally-chosen shape.** The build did **not** add the `relayed_to text[]` per-target ACK set + un-ACKed-subset re-enqueue specced in §3.3 (a schema column + persistent partial-ACK narrowing across retries). Instead, `publishNostrToRelaysDetailed` returns `{eventId, succeeded[], failed[]}`, and `relay-publish.ts` gates the three discovery entity types: a row is `sent` only if **at least one *public* (non-`PLATFORM_RELAY_WS_URL`) fan-out relay ACKed**; an in-house-only ACK routes through `failAndMaybeRetry`, re-publishing the whole event to all targets (idempotent — replaceable kind 0/3/10002). Two deliberate divergences from §3.3's "chosen" design, both for simplicity with no behavioural loss for discovery: (a) **threshold is ≥1 public ACK, not all-targets-ACKed** — avoids one permanently-flaky public relay driving rows to `abandoned`, and replaceable events propagate across the mesh from any single relay; (b) **no schema change / no subset tracking** — retry re-sends to every target, cheap at discovery volume. Non-discovery rows keep the one-accepts rule unchanged. The self-heal sweep (item 2) remains the second durability layer.
...
4. **Follow graph defaults on, with disclosure** (Risk #3 decision). New column `accounts.publish_follow_graph` (default `true`); a settings toggle (`PUT /me/privacy-preferences`) flips it. *(As of 2026-06-10 this toggle, and the `discovery_enabled` "Discoverable on Nostr" opt-in above it, were relocated from the retired `PrivacyPreferences.tsx` into the Nostr row of `NetworkReachPanel.tsx` per NETWORK-CONCIERGE-ADR §10.)* The toggle copy is the user-facing disclosure that the list becomes world-readable. Turning it **off** retracts via an empty kind 3 (`retractFollowList`); turning it **on** marks dirty for republish.
