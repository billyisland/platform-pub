...
**Status:** Implemented (2026-06-14), **surfaces superseded 2026-06-17, model superseded 2026-06-21.** The curated-scheme grammar, derivation logic, and migration mechanism described here are unchanged and current; two things have moved on. (1) The surface set: the `anil`/`vela`/`caju` family was replaced by the `spring`/`summer`/`autumn`/`winter` four-seasons set (one shared frame→ground→surface grammar; `anil`'s broken indigo-walls/umber-interior is what prompted it). (2) The model: a scheme is now a **colourway (seasonal character) orthogonal to light/dark** — each colourway carries a light AND a dark variant and follows the global toggle via `paletteFor(scheme, dark)`, rather than being mode-fixed (`primary`/`dark` collapsed into one `basic` colourway). See **DESIGN-TUNING-FINDINGS §3 + its 2026-06-21 addendum** for the new surfaces, the variant hexes, and the extended `SCHEME_ALIASES` map. Gateway `FEED_SCHEME_IDS` now lives at `gateway/src/routes/feeds/crud.ts`.
