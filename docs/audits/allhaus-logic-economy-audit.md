...
## Wave 2 (2026-07-06) — the remaining deferred items shipped

All six deferred items landed (migrations 142–144, code + tests + schema.sql regenerated, all checks green):

- **F9** — paid voting removed. The `vote_charges` money path is stripped from accrual (convert branch), settlement (advance + chargeback vote arm), payout (eligibility/peek UNIONs, claim, completion advance, rollback, resume EXISTS), and the chargeback planner (`ReversalVote`/`chargeBackVoteIds` gone). Votes now cast free (cost 0, no tab debit, no `vote_charges` row, no ledger entry). The `votes`/`vote_charges` tables + historical `vote_charge` ledger entries are left inert (append-only); the `vote_charge` `LedgerTriggerType` stays so history + the my-account statement still render. Frontend: `VoteControls` casts immediately, `VoteConfirmModal` + web/shared `lib/voting.ts` deleted; `GET /votes/price` removed. Ledger-adjacency registry updated (votes.ts dropped, accrual.ts 3→2). **Residual:** historical `platform_settled`/`accrued` `vote_charges` are no longer settled or paid out — the platform retains that small unpaid vote income (the owner's accepted "leave inert" trade-off).
- **F5** — full publication-aware chargeback. A charged-back publication read now reverses each PAID split recipient of the paying publication payout, prorated by (read gross ÷ payout `total_pool_pence`) against the stored `publication_payout_splits`; the author is never reversed and no earned-side entry fires (pub reads post no `writer_accrual`). Only `initiated|completed` splits are reversed (money that never left the platform isn't). Uses `writer_payout_reversal` (per the audit) — view-correct because `ledger_writer_earnings` sums both `publication_split` (+) and `writer_payout_reversal` (−) per account. **No migration.**
- **F4** — payout completion keyed off the `transfers.create` response (writer/tribute set `completed` at create; publication split `completed` at create, parent completes when all splits done else stays `pending` for the resume sweep). `handleFailed*` guarded on `status != 'completed'`. New `transfer.reversed` handler (migrations 142 payout_status += `reversed`, 143 tribute CHECK) reverses writer/tribute/publication payouts (mark `reversed` + reversing ledger entry, chargeback posture). The misleading webhook comment is fixed. **Per owner decision (2026-07-06): the `transfer.paid/failed` branches are KEPT as guarded no-ops** pending a live-Stripe confirmation before deletion.
- **F10** — publication split caps. `computePublicationSplits` distributes standing shares against the fixed 10000 base (platform retains any unallocated remainder; the sole-1-bps-member-gets-100% renormalisation is gone), clamps per-article cumulative `revenue_bps` at 10000, and floors the pool at 0. Write-side `SUM(bps) ≤ 10000` enforced in the article-override route and the member PATCH route (which bypassed the payroll route's cap).
- **F14** — allowance free/chargeable split modelled explicitly. `classifyRead` returns `allowanceConsumedPence = max(0, min(remaining, amount))`; `read_events.allowance_consumed_pence` persists it (migration 144); the boolean is now derived accurately. Coordinated with the shipped F3 hard gate: the allowance decrement stays the FULL amount (the metering counter must drift to the F3 floor — capping it would re-open the unmetered-read hole), so F14's literal "cap the decrement" is deliberately not adopted; the split lives in the pence column.
- **Wave 5** — `resumePendingSettlements` now runs each reconcile cycle (not only at startup); `subscription-expiry` renewal uses `setUTCMonth`/`setUTCFullYear` calendar arithmetic; the `read_at <= settled_at` approximate-pairing note added to `confirmSettlement`.
