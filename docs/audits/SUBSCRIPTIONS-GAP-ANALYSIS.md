...
> **Reconciliation status (2026-05-15):** Phase 1 is complete — all 5 MVP items and both "should include" items are resolved. Auto-renewal (Step 1) ships in `gateway/src/workers/subscription-expiry.ts` with offer-period handling, Nostr kind 7003 attestation via relay outbox, and expiry-warning emails. UI gaps (Step 2) resolved across `/ledger` (reader management), dashboard Pricing tab (writer price setting), PaywallGate (subscribe prompt + spend-threshold nudge), and subscription-events endpoint. Subscription emails (Step 3) ship four templates in `shared/src/lib/subscription-emails.ts`. Annual pricing (Step 4) and comp subscriptions (Step 5) both live. Phase 2 was not started at the time of writing — free trials, gift subscriptions (purchaser→recipient flow), welcome email on subscribe, subscriber import/export, growth analytics, and custom subscribe landing page (`/[username]/subscribe`) were all outstanding. **Superseded in part by the two notes below: grant-mode gifting (2026-08-04) and Step 8's welcome email (2026-08-18) have since shipped.**
>
> **Renewal hardening (2026-06-03):** an audit of the renewal worker against this spec found and fixed four divergences (`gateway/src/workers/subscription-expiry.ts`, +`tests/subscription-expiry.test.ts`): (1) **publication subscriptions never renewed** — the renewal SELECT inner-joined `accounts ON writer_id`, dropping every `publication_id`-targeted sub; since Phase-2 expiry only touches `auto_renew = FALSE`, those subs sat active past period end with no further charge (free perpetual access). Now `LEFT JOIN`s both targets, COALESCEs the writer pubkey over the publication, and routes the earning to `publication_id` (mirroring the subscribe path's quirk where `subscription_events.writer_id` holds the publication id). (2) **Reading-tab clamp** — renewal deducted with `GREATEST(free_allowance_remaining_pence - price, 0)`, silently undercharging readers below the price while still crediting the writer in full; the tab is designed to accrue negative (settled via Stripe), so it now deducts without a floor, matching both subscribe paths. (3) **Annual-discount hardcode** — offer-expiry revert used `* 12 * 0.85` instead of the writer's configurable `accounts.annual_discount_pct`. (4) **Retry-once** — renewal now retries the atomic transaction once before expiring, so a transient DB/signing blip no longer permanently kills a paid subscription (per Step 1's "retry once, then expire"). Carried (no behaviour change, flagged for a product call): the worker charges once per missed period per hourly tick when catching up after downtime — a thundering catch-up burst is possible, vs. advancing to the next future boundary in one step.
>
> **Publication-sub charge logging (2026-06-04, migration 103):** live verification of the above surfaced a deeper pre-existing bug — `subscription_events.writer_id` was `NOT NULL` FK→`accounts`, so logging a *publication* subscription's charge (publication id in the writer slot) violated the FK on **both** the subscribe path (`routes/subscriptions/publication.ts`) and the renewal worker; it had never fired because there are 0 publication subs in the DB. Migration `103_subscription_events_publication.sql` adds a nullable `publication_id` (FK→`publications`), relaxes `writer_id` to nullable, and adds a `writer_id OR publication_id` CHECK; `logSubscriptionCharge` now takes a `publicationId` and writes the right column. Live-verified that a publication sub renews (charge/earning logged against `publication_id`, period rolled forward).
>
> **Period arithmetic and grant mode (2026-08-04, migrations 170–171):** Step 1's instruction to "roll `current_period_start` and `current_period_end` forward 30 days" is **superseded** — a month is a calendar month, and the boundary is anchored. Two defects closed (CONSOLIDATED-TODO §1.5): every create path (subscribe, re-activate, publication-subscribe, comp) added a literal 30 or 365 days, and the renewal worker — which did use calendar arithmetic — advanced the *previous* period end with `setUTCMonth`, which overflows rather than clamps (31 Jan + 1mo → 3 Mar) and then advanced the drifted value again, so a subscriber who signed up on the 31st walked forward every month. The anchor is now stored (`subscriptions.period_anchor_day`, migration 170) because it cannot be recovered from a clamped end, and the arithmetic has one home in `shared/src/lib/subscription-period.ts`. Existing drift is deliberately frozen rather than corrected: re-deriving the true anchor would push a live subscriber's renewal later than the date they were already told. **Phase 2's "gift subscriptions" item is now PARTLY resolved, and the distinction matters:** grant-mode offers — a *writer* addressing a discounted or free subscription at one named reader — were modelled end to end but created with `code = NULL` while the redeem lookup filtered `mode = 'code'`, so they had no URL and no redemption; both modes now carry a code and the mode filter became a recipient check (§1.10, migration 171). What is still **not** built is the Substack-shaped **purchaser→recipient** gift, where a *reader* pays for someone else's subscription — that needs a payer distinct from the beneficiary and is untouched by this work. Still carried from 2026-06-03: the catch-up multi-charge product call (§1.7).

>
> **Step 8 — the welcome email is BUILT (2026-08-18, migration 180):** `accounts.subscription_welcome_message`, `sendSubscriptionWelcomeEmail` in `shared/src/lib/subscription-emails.ts`, `GET`/`PATCH /settings/subscription-welcome`, and a *Welcome message* panel on the dashboard's Pricing tab (CONSOLIDATED-TODO §4.2). The step as specified said "writer-configurable welcome message, sent automatically on subscribe, default template if the writer hasn't customised, preview in settings" — all of that ships except the **preview**, which the panel does not have and which is a small follow-up rather than a gap in the mechanism. Three departures from the spec as written, each deliberate. **The message is plain text and escaped at send, never stored as markup** — it is a writer-authored string reaching every one of their subscribers' inboxes, so storing HTML would make it an injection surface for nothing the template does not already supply; `welcomeParagraphs` turns blank lines into paragraphs and single newlines into `<br>`, so a writer's list stays a list. **NULL and `''` are stored distinctly** — NULL means "never set one" and both send the default today, but keeping them apart gives a later "send nothing at all" opt-out a value to hang on without a second column. **And it fires on BOTH subscribe paths**, create and reactivate: a reader returning after a lapse is being welcomed back and is owed the writer's words as much as a first-timer. Two things the step did NOT reach, recorded so neither reads as an oversight: **publication subscriptions** (a publication has no `accounts` row to hang the message on — it wants a `publications` column or a shared home, a design call rather than a copy-paste), and **the comp path** (`POST /subscriptions/:readerId/comp`), where the gap is older and bigger — it sends *no* email at all, so a comped reader is granted a subscription and never told, and whether "You're subscribed to X" is the right thing to send someone who did not ask to be is its own question.
>
> **A note for Step 10 (subscriber analytics), which the queue lists next:** `subscription_events.event_type` has no `subscribed`/`cancelled` member — its CHECK allows only `subscription_charge`, `subscription_earning`, `subscription_read` and `expiry_warning_sent`. So growth and churn come off `subscriptions.started_at` / `cancelled_at`, **not** off the event log, and Step 10's claim that "`subscriptions` and `subscription_events` tables already have everything needed" is true only in that first sense. Reading the event log as a lifecycle stream is the trap.
...
**Step 8: Welcome email** — **BUILT 2026-08-18 (migration 180)**; see the reconciliation note at the top of this file for the three deliberate departures and the two paths it does not reach.

- ~~Writer-configurable welcome message (plain text, stored in `accounts.subscription_welcome_message`)~~ — shipped at the column this step named, and plain text turned out to be the security posture rather than a simplification: the value is escaped at send, never stored as markup.
- ~~Sent automatically on subscribe~~ — on **both** subscribe paths, create and reactivate.
- ~~Default template if writer hasn't customised~~ — and NULL ("never set") is kept distinct from `''` ("cleared"), so a later send-nothing opt-out has a value to hang on.
- **Preview in settings** — NOT built. The panel is a textarea and a character count; there is no rendered preview. Small follow-up, not a gap in the mechanism.
