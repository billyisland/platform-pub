...
**Status:** Part 1 (§1.1–§1.8) accepted, ready to scope for build. **Shipped so far:** item A (settlement lock-order fix, 2026-07-13); **§1.8 `applyLedgerDelta` + §1.3(1) dispute-stake tests (2026-07-14); §1.4 chargeback-attribution policy paragraph + §1.5 tax-schema migration 155 (2026-07-14); the Dial-A rework (§2.2 prerequisite — migration 156, 2026-07-14).** The whole "this week (decision-independent)" build queue is shipped, and the scope-first prerequisite (Dial-A rework) is now done — so §2.2 is dissolved in code as well as on paper, and only §2.3 (counsel sign-off) remains this-week. Part 2: §2.2 decided (Dial A, 2026-07-13) **and shipped**; §2.1 and §2.3 open, pending counsel.
...
**Before launch:** §1.1 battery → primitive extraction (writer → settlement → publication → tribute, whose battery is now writable against the shipped post-Dial-A shape); §1.2 reconciliation job (alert + halt on mismatch); §1.3(2) pledge tests + §1.3(4) traffology math (§1.3(3) paid-DM and gift links dropped per §B); §1.6 merchant-posture module, greenfield (§B3).
