...
**Status:** Part 1 (§1.1–§1.8) accepted, ready to scope for build. **Shipped so far:** item A (settlement lock-order fix, 2026-07-13); **§1.8 `applyLedgerDelta` + §1.3(1) dispute-stake tests (2026-07-14); §1.4 chargeback-attribution policy paragraph + §1.5 tax-schema migration 155 (2026-07-14); the Dial-A rework (§2.2 prerequisite — migration 156, 2026-07-14); §1.1 step 1 — the saga conformance battery for all four flows (2026-07-14).** The whole "this week (decision-independent)" build queue is shipped, and the scope-first prerequisite (Dial-A rework) is now done — so §2.2 is dissolved in code as well as on paper, and only §2.3 (counsel sign-off) remains this-week. On the "before launch" queue, §1.1 **step 1 (battery)** is now shipped; **step 2 (primitive extraction)** is the next code item, de-risked by the green battery baseline. Part 2: §2.2 decided (Dial A, 2026-07-13) **and shipped**; §2.1 and §2.3 open, pending counsel.
...
8. **[build]** §1.1 step 2 — **saga** primitive extraction (`executeStripeIdempotent`, `statusGuardedTransition`), one flow per commit, order **writer → settlement → publication → (tribute, after Dial-A)** — multi-leg publication last, after both saga shapes are proven. **The battery (item 7) is the green baseline this must stay green against.** (`applyLedgerDelta` already shipped as item 2 — it is not part of this pass.)
...
**Before launch:** §1.1 battery ✅ SHIPPED 2026-07-14 (all four flows, 32 tests, suite 143/143 — see FIX-PROGRAMME) → **next: primitive extraction** (writer → settlement → publication → tribute, whose battery was written against the shipped post-Dial-A shape; the green battery is the baseline it stays green against); §1.2 reconciliation job (alert + halt on mismatch); §1.3(2) pledge tests + §1.3(4) traffology math (§1.3(3) paid-DM and gift links dropped per §B); §1.6 merchant-posture module, greenfield (§B3).
