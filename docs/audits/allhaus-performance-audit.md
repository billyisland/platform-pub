...
1. **SSR `/author`, `/tag`, `/source`** (#5 residual). Each `page.tsx` is a server shell rendering a `'use client'` body (`AuthorProfileView` / `TagBrowser` / `SourceSurface`) that fetches client-side. Lift the first data fetch into the server `page.tsx` and pass it down as props (the `/article/[dTag]` page is the reference), add `next: { revalidate: 60 }`, and keep the interactive bits (infinite scroll, follow state) working in the client child. Confirm none vary by viewer before caching. Medium task **per page**, independent of each other.
   - **`/tag` DONE (2026-06-20).** `GET /tags/:name/posts` is `optionalAuth` and viewer-independent (article cards, global vote counts; bookmark state hydrates client-side), so the server `page.tsx` now fetches it anonymously with `revalidate: 60` and seeds `TagBrowser` via `initialItems`/`initialTotal`/`initialCursor`; the client skips its first fetch when seeded (the surface overlay passes none and keeps client-fetching with the viewer cookie). `next build` ⇒ `/tag/[tag]` is `ƒ` server-rendered. NOT browser-verified (web is a baked prod image).
   - **`/author` + `/source` BLOCKED on a product call.** Both read endpoints (`GET /author/:id/profile`+`/posts`, `GET /sources/:id`) are `requireAuth` and viewer-relative (follow state, identity links) — so an anonymous SSR fetch gets 401 (no public data to render) and a `revalidate` shared cache would be wrong. Doing these "the article way" requires first converting the endpoints to `optionalAuth` (serve public content to anon, omit viewer bits) — which **publicly exposes author post-logs / follower counts / source item lists to anonymous crawlers**, a privacy/product decision. Until that's made, the SSR benefit is marginal (logged-in visitors are `WorkspacePaneRedirect`'d to the overlay anyway). Deferred deliberately.
