...
8. **Discovery expansion (design accepted 2026-07-10; Phases 0+1+2 SHIPPED 2026-07-10): `docs/adr/RESOLVER-DISCOVERY-ADR.md`.** Phase 1 (known-world pg_trgm index over `external_authors`/`external_sources`, migration 150, `searchKnownWorld` in Phase A) and Phase 2 (`activitypub_discovery` — unauthenticated Mastodon `/api/v2/search` behind the FASP-ready `ApAccountSearchProvider` interface, `MASTODON_DISCOVERY_INSTANCES`; plus the §5.2 addSource acct→webfinger amendment, a down-payment on F1) shipped — see FIX-PROGRAMME 2026-07-10. Remaining phases: 3 = bridge-aware merge (helpers → `shared/`) + confidence-tier ordering/rendering — **discharges F3 and F5.4 above**; 4 = generated catalog (Wikidata/OPML, ~300–500 probed entries) — **discharges F7**. Branch 4 web-search bridge stays deferred behind `DISCOVERY_WEBSEARCH_ENABLED`; Feedly ruled out (API ToS).
