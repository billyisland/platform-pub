...
7. **F8:** resolver orchestration/network-path tests are zero — prerequisite for the `resolver.ts` decomposition already queued in §8.5 **and Phase 0 of the discovery expansion (next item)**.
8. **Discovery expansion (design accepted 2026-07-10, unbuilt): `docs/adr/RESOLVER-DISCOVERY-ADR.md`.** Phases: 0 = the F8 orchestration harness; 1 = known-world pg_trgm index over `external_authors`/`external_sources` (migration 150); 2 = `activitypub_discovery` (unauthenticated Mastodon `/api/v2/search`, behind a FASP-ready provider interface); 3 = bridge-aware merge (helpers → `shared/`) + confidence-tier ordering/rendering — **discharges F3 and F5.4 above**; 4 = generated catalog (Wikidata/OPML, ~300–500 probed entries) — **discharges F7**. Branch 4 web-search bridge stays deferred behind `DISCOVERY_WEBSEARCH_ENABLED`; Feedly ruled out (API ToS).
