...
## 0. 2026-07-09 commit audit — July 7–8 ships (the HIGHs block the next deploy)

Four-agent adversarial review of commits `3b2d51d`…`515a13b` (external-author-history Phases 1–4, paywall hardening, draft dedup + migration 149, Blossom cutover, Stripe webhook route). The paywall commit (`339b43e`) and the external-author-history cluster verified **clean** — money paths, validator lockstep, Step-1b ordering, promotion/shadow-source/job-key/SSRF/relay-free invariants all hold. Findings, worst first:

1. **HIGH — public Blossom upload through nginx (live on prod).** The `nginx.conf` `location /media/` block proxies *every* method and path to Blossom, so `PUT https://all.haus/media/upload` reaches the upload endpoint from the open internet. BUD-02 `requireAuth` accepts *any* self-signed kind-24242 event and `blossom-config.yml` carries no pubkey allowlist — its own comment says internal-only reachability *is* the access control, which this proxy invalidated. Anyone can mint a keypair and host arbitrary `image/*` blobs (≤1 MB) at `https://all.haus/media/<sha>.webp` with 1-year immutable cache headers (phishing/abuse-content liability). **Fix:** `limit_except GET HEAD { deny all; }` in the `/media/` block, or match only the hash-shaped read path (`location ~ ^/media/[0-9a-f]{64}(\.[a-z0-9]+)?$`); then the prod nginx force-recreate. → `9664366`.
2. **HIGH — the first-save guess path can still overwrite a scheduled draft** (the exact bug `d34f9c0`'s message claims closed). `gateway/src/routes/drafts.ts:110-115`: the untagged-draft guess SELECT has no `scheduled_at IS NULL` filter, so scheduling article A then starting new article B lets B's first autosave pick A's waiting draft as most-recent-untagged and COALESCE-overwrite it — the scheduler publishes B's content in A's slot, then deletes the row; A's content is gone. **Fix:** add `AND scheduled_at IS NULL` to the guess SELECT.
3. **HIGH — drive fulfilment fires at the v1 index step; a vault failure charges pledgers for a never-published article.** `web/src/lib/publish.ts:69` now sends `draftId` on *both* index calls and the index route (`gateway/src/routes/articles/publish.ts:187`) awaits `checkAndTriggerDriveFulfilment` — so a paywalled publish matches the drive at step 2 (status→`published`, `article_id` stamped, `fulfillDrive` pledge-charging kicks — `drives.ts:697-723`) *before* the vault seals. If step 3/4 fails, the article soft-deletes but nothing unwinds the drive: pledgers charged for undelivered content, and a retry publish can never re-match. Vault failure is a live failure mode (the 2026-07-07 poisoned-article bug). **Fix:** send `draftId` only on the *final* index call (v2 / free), or defer fulfilment until the publish pipeline completes.
4. **MEDIUM — a swallowed fulfilment error + migration 149 = permanently orphaned drive.** All three `checkAndTriggerDriveFulfilment` call sites (`articles/publish.ts:187`, `scheduler.ts:171`/`:393`) `.catch(log)`; the caller then deletes the draft and `ON DELETE SET NULL` nulls `pledge_drives.draft_id` — the sole key the match uses — so a drive whose match txn failed once sits `open`/`funded` forever, pledges never processed (pre-149 the FK error forced a retry loop; recoverable, now silent). **Fix:** a fulfilment failure must block the draft delete (the scheduler already tolerates retry).
5. **MEDIUM — the orphaned-draft cleanup script can delete an in-review submission's draft.** `scripts/cleanup-orphaned-drafts.sql` Tier 1 joins on `a.deleted_at IS NULL AND a.title = d.title` with no `a.published_at IS NOT NULL` filter: a pending publication submission (real `articles` row with `published_at` NULL and body = the draft body) classifies Tier 1 and is auto-deleted under `-v apply=true` — exactly the draft the code fix deliberately keeps; if the submission is then rejected, the writer's only copy is gone. **Fix the predicate BEFORE the §11 prod cleanup run.**
6. **LOW — publish emails fire before a paywalled publish completes.** The step-2 v1 index (`web/src/lib/publish.ts:80` → `gateway/src/routes/articles/publish.ts:196-211`) triggers `sendPublishNotifications` for a NEW article before vault/v2; the new failure soft-delete converts a step-3/4 failure into permanent 404 links in subscriber inboxes (previously: live article, broken paywall). **Fix:** suppress `sendEmail` at step 2 for paywalled articles and fire after step 5 (needs a small route affordance — the step-5 upsert is never `isNew`). → `339b43e`.
7. **LOW — Blossom rollback claim overstated (docs).** Post-cutover blobs exist only in `blossom_data` (`media.ts` no longer writes to disk), so the "revert → disk backend serves every blob" comment is false: a revert 404s all post-cutover media and no Blossom→disk copy script exists. Amend the ADR/compose rollback notes (or write the reverse script) before leaning on the soak-cycle rollback.
8. **Cosmetic tail (batch with §7):** atproto/AP promotion overwrites a hydrated `feed_items.author_name` with the null fallback pre-enrichment (`feed-ingest/src/lib/atproto-ingest.ts:129-141`; byline unaffected — reads `external_items` first — and enrichment self-heals); unguarded `post.record?.reply?.parent.uri` aborts one hydration pass on a malformed AppView entry (`gateway/src/lib/author-timeline-hydration.ts:274`); `stampGuard` evicts oldest-inserted, not expired-first (`author-timeline-hydration.ts:84-90`); `autoSaver.cancel()` clears only the debounce timer — the editor stays typeable during the multi-second publish, so a mid-publish keystroke can re-arm an autosave that lands after the draft delete and re-mints an orphan (narrow window, same bug class d34f9c0 fixes); the approve path never disposes of the kept in-review draft (draft+article shadow persists for approved submissions — decide scope or fix); stale "source_item_uri bakes in relay_urls" comment in `feed-ingest/src/lib/nostr-ingest.ts` (~378, false since migration 101); `PaywallGate` copy conservatively wrong iff `FREE_ALLOWANCE_FLOOR_PENCE` ≠ 0 (default correct); PATCH `/articles/:id` (`gateway/src/routes/articles/manage.ts:103`) still returns a raw `flatten()` as `error` (part of the §7.17/§8.9 error-shape batch).
...
0. **§0 commit-audit HIGHs + the two MEDIUMs** — one short session: nginx `/media/` method restriction (prod-live exposure), `scheduled_at IS NULL` on the draft guess, drive-fulfilment timing + error handling, cleanup-script predicate. All small, all before the next deploy / the §11 prod cleanup runs.
