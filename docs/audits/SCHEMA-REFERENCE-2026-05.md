...
| `idx_notifications_dedup`      | `notifications`       | UNIQUE `(recipient_id, actor_id, type, COALESCE(article_id/note_id/comment_id/offer_id/drive_id, sentinel))` | `read = false` | **At most one UNREAD notification per tuple** — reading one frees its slot so a repeat event notifies again. Every `INSERT INTO notifications` is a bare `ON CONFLICT DO NOTHING`, so this index alone decides what collapses. Partial only from **migration 173 (2026-08-05)**: migration 019 wrote the clause in 2023 but was seeded as applied by `schema.sql`, which carried the non-partial form, so it never ran on any DB. `drive_id` joined in **174** (same day) — a reference column is what stops two different targets colliding, so a type whose column is missing from this list dedups across targets. `actor_id` is deliberately **not** COALESCEd (NULL actors must never collide — `pledge_fulfilled`) |
...
| `idx_notifications_drive`        | `notifications`       | `(drive_id)`                      | `drive_id IS NOT NULL`               | Drive-triggered notification lookup (migration 174) |
