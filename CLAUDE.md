...
- **Feed rhythm**: in the live workspace each unified `PostCard` carries its own `marginBottom` (no wrapping `space-y`), governed by `GAP_PX` in `web/src/lib/post/level-spec.ts` — **10px** between independent feed items, **5px** between items in an expanded conversational chain (ancestors · focal · replies, a uniform beat). No horizontal rules between cards (see No hairlines — absolute sitewide). (The retiring legacy `/feed`+`/source` cards in `components/feed/` use `space-y-[10px]`; the legacy `PlayscriptThread` still uses `space-y-[32px]` — see Reply threads below.)
