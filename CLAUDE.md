...
- Engagement counts: `external_items` carries denormalised `like_count`, `reply_count`, `repost_count` (migration 090). Populated at ingest time for Bluesky backfill (from `getAuthorFeed` response); Jetstream, AP outbox, and RSS items start at 0. `external_engagement_refresh` cron (`*/30 * * * *`) batch-fetches fresh counts from source platforms for items published in the last 7 days — Bluesky via `getPosts` (25/call), Mastodon via `GET /statuses/:id` per instance. `ExternalVesselCard` renders counts as heart/speech-bubble/repost icons in mono-caps 11px; hidden when all zero. Expanded view renders `contentHtml` via `dangerouslySetInnerHTML` with `[&_p]:mb-2` paragraph spacing (fixes RSS paragraph break loss). Phase 2: `GET /api/v1/external-items/:id/engagement` fetches live counts on card expand (30s in-memory cache, side-effect updates snapshot columns); `useLiveEngagement` hook drives the `EngagementRow` with live values
...
- `docs/adr/CARD-BEHAVIOUR-ADR.md` — unified card interaction model (click regions, conversational neighbourhood expansion, biddability tiers, author affordances). Phases 1–5 complete. External card bylines route to the internal **source surface** (`/source/[id]` page → `GET /api/v1/sources/:id` in `gateway/src/routes/sources.ts`), not the origin platform — the origin link lives on the source-attribution line. Neighbourhood hydration (`/external-items/:id/thread` + `/parent`) is rate-limited (30/min), 5s-timeout-capped, and returns a server-signalled `partial` flag. Phase 5 (2026-05-30) brought the **workspace** vessel cards (`web/src/components/workspace/VesselCard.tsx`) in line with this model — body-click expands the neighbourhood, the source-attribution line is the one route out (the `Open original` button is gone), and the byline links to the source surface; the `/feed/` cards (`ExternalCard`/`NoteCard`/`ArticleCard`) are being retired in favour of the workspace surface. Only the §VI.3 constructed external author profile remains deferred (its own ADR)
