...
- Universal resolver (`gateway/src/lib/resolver.ts`) provides identity resolution for subscribe, invite, and other input fields — see `POST /api/v1/resolve`. Async Phase B results persist in `resolver_async_results` (migration 061, initiator-bound, pruned every 5m by `resolver_results_prune`) so poll results survive gateway scale-out. AT Protocol identity via `gateway/src/lib/atproto-resolve.ts` (DID docs, handle resolution, profile metadata — all through the public AppView `public.api.bsky.app`)
...
- `vouches` (migration 066): per-attestor/subject/dimension endorsements. Dimensions: `humanity`, `encounter`, `identity`, `integrity`. Values: `affirm`/`contest`. Visibility: `public` (attributable on profiles, feeds Layer 4) or `aggregate` (anonymous to readers). Contests must be aggregate. One vouch per attestor/subject/dimension (upsert). Soft-delete withdrawal via `withdrawn_at`
- `trust_profiles` (migration 066): precomputed dimension scores — empty until Phase 4 epoch aggregation cron ships
- Gateway `trust.ts`: `GET /trust/:userId` (Layer 1 + Layer 2 dimension scores + public endorsements + Layer 4 relational data + viewer's vouches), `POST /vouches` (create/upsert), `DELETE /vouches/:id` (withdraw), `GET /my/vouches` (own vouch list)
...
- Frontend: `TrustProfile` component (dimension bars, endorsements, Layer 4), `VouchModal` (dimension selector, visibility radio, aggregate disclaimer), vouch button on writer profiles, `VouchList` on `/network?tab=vouches` with withdrawal
