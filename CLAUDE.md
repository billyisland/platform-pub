...
- **Relay outbox (§60, programme one complete — Phases 1–3 + 5 + 6 landed)**: signed events destined for the platform relay are enqueued into `relay_outbox` (migration 076) inside the caller's transaction via `shared/src/lib/relay-outbox.ts::enqueueRelayPublish`. The `feed-ingest` worker `relay_publish` publishes via `publishNostrToRelays`, owns retry with `attempts` / `next_attempt_at` / `max_attempts`, and uses a transaction-scoped advisory lock on `(entity_type, entity_id)` for per-entity serialisation. A minute-cadence `relay_outbox_redrive` cron provides a second heartbeat independent of the enqueue path; `relay_outbox_reconcile` emits daily queue metrics. All 13 non-publish-path call sites (subscriptions create/reactivate/cancel/renew, conversation pulse, account/article/note/publication/drive deletions, `POST /sign-and-publish` passthrough, payment-service kind-9901 receipts) sign locally and hand the signed event to `enqueueRelayPublish` inside the caller's txn — `publishSubscriptionEvent` is now `signSubscriptionEvent`, `publishReceiptEvent` is `signReceiptEvent`, and payment-service shares the shared outbox rather than its own ws publisher. `POST /sign-and-publish` semantic change: 200 means "signed and durably queued", not "on relay". Phase 5 (scheduler §1 contortion): `workers/scheduler.ts::publishPersonalDraft` no longer calls `publishToRelay` — free drafts enqueue v1 in the same txn as the article INSERT; paywalled drafts keep txn 1 (article row as vault-ownership anchor) but txn 2 now UPDATE-s to v2.id *and* enqueues v2 atomically, closing the crash window between DB commit and relay publish. Phase 6: `feed-ingest/src/tasks/relay-publish.test.ts` covers success / failure-with-retry / abandon-at-max-attempts / advisory-lock contention / already-sent no-op plus a two-step deletion-path retry (relay throws, then succeeds). Phase 4 (publish-path rewrite — `publishToRelay` still live in `services/publication-publisher.ts`) is the only outstanding piece and is deferred to its own follow-up ADR. Spec: `docs/adr/RELAY-OUTBOX-ADR.md`
