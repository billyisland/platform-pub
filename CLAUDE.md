...
- **Relay outbox (§60, complete)**: every write path that publishes a signed Nostr event to the platform relay enqueues into `relay_outbox` (migration 076) inside the caller's transaction via `shared/src/lib/relay-outbox.ts::enqueueRelayPublish`. The `feed-ingest` worker `relay_publish` publishes via `publishNostrToRelays`, owns retry with `attempts` / `next_attempt_at` / `max_attempts`, and uses a transaction-scoped advisory lock on `(entity_type, entity_id)` for per-entity serialisation. A minute-cadence `relay_outbox_redrive` cron provides a second heartbeat independent of the enqueue path; `relay_outbox_reconcile` emits daily queue metrics. Phases 1–3 + 5 migrated the non-publish-path call sites (subscriptions create/reactivate/cancel/renew, conversation pulse, account/article/note/publication/drive deletions, `POST /sign-and-publish` passthrough, payment-service kind-9901 receipts, and scheduled personal drafts); Phase 4 migrated the two publication publish sites — `publishToPublication` and `approveAndPublishArticle` — under the eager-commit shape (sign → one txn does INSERT/UPDATE article + feed*items + `enqueueRelayPublish` → 201; relay blips become invisible worker retries instead of 5xx). `publishSubscriptionEvent` is now `signSubscriptionEvent`; `publishReceiptEvent` is `signReceiptEvent`; payment-service shares the shared outbox rather than its own ws publisher. `POST /sign-and-publish`, `POST /publications/:id/articles`, and `POST /publications/:id/articles/:articleId/publish` all now mean "signed and durably queued", not "on relay". Phase 5 also closed the scheduler §1 hazard — paywalled drafts keep txn 1 (article row as vault-ownership anchor) but txn 2 UPDATE-s to v2.id \_and* enqueues v2 atomically. Phase 6: `feed-ingest/src/tasks/relay-publish.test.ts` covers success / failure-with-retry / abandon-at-max-attempts / advisory-lock contention / already-sent no-op plus a two-step deletion-path retry. `publishToRelay` / `publishToRelayUrl` are deleted from `gateway/src/lib/nostr-publisher.ts`. Specs: `planning-archive/RELAY-OUTBOX-ADR.md` + `planning-archive/RELAY-OUTBOX-PHASE-4-ADR.md`
...
- External content (RSS/Atom/JSON Feed, external Nostr, Bluesky, Mastodon/Lemmy/threadiverse, email newsletters) is ingested by `feed-ingest/` (Graphile Worker service + a long-lived Jetstream WebSocket listener, no HTTP port). The RSS adapter (`feed-ingest/src/adapters/rss.ts`) auto-detects JSON Feed responses (`application/feed+json` content-type or shape-sniff) and parses them natively, falling through to `rss-parser` for RSS/Atom. Podcast feeds (RSS with `itunes:*` / `podcast:*` namespaces) are enriched: audio enclosures carry `duration_in_seconds`, `size_in_bytes`, and episode artwork (`thumbnail`); `interaction_data` captures `chaptersUrl`, `transcriptUrl`, `episode`, `season` from the Podcasting 2.0 namespace. `ExternalCard` renders audio items with a native `<audio>` player and duration label
...
- Mastodon / Lemmy / threadiverse (Phase 4 + Slice 2, read-only): `feed-ingest/src/adapters/activitypub.ts` fetches the actor doc to find the outbox URL, then paginates the outbox newest-first. Public `Create` activities with object type `Note`, `Article`, or `Page` are ingested (visibility check via `to`/`cc` containing the AS Public URI); Announces (boosts), non-public posts, and tombstones are ignored — outbox polling has no deletion signal, so delete propagation waits for inbox delivery (future phase). `Page` type support enables Lemmy/PieFed/Mbin posts; the adapter extracts `name` as title and `audience` as community context into `interactionData`. `feed-ingest/src/tasks/feed-ingest-activitypub.ts` is the per-source poll task, cursor = id URI of the previous newest item. `feed-ingest/src/lib/activitypub-ingest.ts` shared dual-write (writes `title` to both `external_items` and `feed_items`) + `activitypub_instance_health` counters (see migration 056). WebFinger + actor lookup: `gateway/src/lib/activitypub-resolve.ts` (`resolveWebFinger`, `fetchActorProfile`, `extractFromMastodonUrl`, `extractFromThreadiverseUrl`). `extractFromThreadiverseUrl` detects Lemmy `/c/`, `/u/` and Mbin `/m/` URL patterns, returning a WebFinger-ready handle. The universal resolver's `fediverse_handle` chain is live, and the ambiguous `user@domain` chain runs WebFinger alongside NIP-05. Mastodon sources wear a `BETA` label in `SubscribeInput` and the subscriptions page — per ADR §XIII, outbox polling is best-effort. Engagement refresh and parent prefetch use Mastodon REST API endpoints — these fail gracefully for non-Mastodon AP instances (Lemmy engagement counts stay at ingest-time values)
- Email newsletters (Slice 3, push-only): `gateway/src/routes/inbound-mail.ts` receives Postmark inbound webhook at `POST /inbound-mail/:secret` (path-token auth via `INBOUND_MAIL_SECRET`). Looks up `external_sources.ingest_address` to match recipient → source, enqueues `feed_ingest_email` task. `feed-ingest/src/adapters/email.ts` normalises Postmark payload: newsletter-specific HTML sanitisation strips tracking pixels (1×1 images, known tracker domains), collapses table layouts, strips MSO conditional comments, then runs through `sanitizeContent()`. `extractCanonicalUrl()` finds "view in browser" links for cross-source dedup. `feed-ingest/src/lib/email-ingest.ts` dual-write helper with two dedup layers: canonical URL match across user's sources (catches newsletter-to-RSS overlap), then title+date fuzzy match (±1 hour). Subscribe flow (`gateway/src/routes/external-feeds.ts`) generates `<sourceId>@ingest.all.haus` ingest address, returns it with copy-to-clipboard UI in `SubscribeInput`. Email sources excluded from poll dispatch (push-only). `protocol = 'email'`, `tier = 'tier4'`. No engagement counts, no threading, no write path. Migration 096 adds `ingest_address` on `external_sources` and `canonical_url` on `external_items`
- Engagement counts: `external_items` carries denormalised `like_count`, `reply_count`, `repost_count` (migration 090). Populated at ingest time for Bluesky backfill (from `getAuthorFeed` response); Jetstream, AP outbox, and RSS items start at 0. `external_engagement_refresh` cron (`*/30 * * * *`) batch-fetches fresh counts from source platforms for items published in the last 7 days — Bluesky via `getPosts` (25/call), Mastodon via `GET /statuses/:id` per instance. That same Mastodon `/statuses/:id` fetch also reads the status's OpenGraph `card` (a Mastodon-API-only object, never in the AP outbox) and merges it into `external_items.media` as a `{type:"link"}` entry (`cardToLinkMedia`/`mergeLinkMedia`), so Mastodon link previews render through the same `MediaBlock` path as Bluesky's `app.bsky.embed.external` (which already lands in `media[]` at ingest). `ExternalVesselCard` renders counts as heart/speech-bubble/repost icons in mono-caps 11px; hidden when all zero. Expanded view renders `contentHtml` via `dangerouslySetInnerHTML` with `[&_p]:mb-2` paragraph spacing (fixes RSS paragraph break loss). Phase 2: `GET /api/v1/external-items/:id/engagement` fetches live counts on card expand (30s in-memory cache, side-effect updates snapshot columns); `useLiveEngagement` hook drives the `EngagementRow` with live values
- Parent context (Phase 2, migration 091): `external_items.is_context_only` marks parent posts fetched purely as reply context (excluded from `feed_items`). `GET /api/v1/external-items/:id/parent` returns the parent item + optional grandparent tag; fetches from source platform on miss, stores as context-only. `external_parent_prefetch` task in feed-ingest eagerly fetches parents at ingest time (enqueued by Jetstream listener, atproto backfill, and AP poll tasks when `source_reply_uri` is non-null). `ParentContextTile` component renders parent above the reply content when an external item is expanded
- Rich embeds — quoted posts (Bluesky + Mastodon, CARD-BEHAVIOUR-ADR addendum 2026-05-30): a quote post renders as a nested mini-card (`QuotedPostTile`) showing the quoted author byline (plain text — never routes out, never a fabricated `/source` link), text, and the quoted post's own media. Mirrors the parent-context architecture exactly: the quote URI lives on `external_items.source_quote_uri` (Bluesky already captured it; `feed-ingest/src/adapters/activitypub.ts::extractQuoteUri` now reads FEP-044f `quote` / Fedibird `quoteUrl` / Misskey `_misskey_quote`); `external_parent_prefetch` also eagerly hydrates quotes (enqueue sites fire on `source_reply_uri` **or** `source_quote_uri`); and `GET /api/v1/external-items/:id/quote` lazily fetches on a cold miss via `fetchBlueskyQuote`/`fetchMastodonQuote` (same 30/min rate-limit, in-memory cache, and server-signalled `partial` flag as `/parent`), storing the quoted post as a context-only row with its own media. The Bluesky view-media extractor reads `images`/`external`/`video` `#view` embeds and the `media` side of `recordWithMedia` but does not recurse into a quote-of-quote. Link previews render via `MediaBlock`'s `LinkPreviewCard` from `media[]` `{type:"link"}` entries. Nostr/RSS rich embeds deferred
- Thread expansion (Phase 3): `GET /api/v1/external-items/:id/thread` returns `{ ancestors, descendants }` as flat `ExternalThreadEntry[]` arrays. Bluesky via `getPostThread` (depth 50, parentHeight 100, recursive tree flattened BFS + chronological sort); Mastodon via `GET /statuses/:id/context` (maps directly). Nostr/RSS return empty (deferred). 60s in-memory cache. `useExternalThread` hook (module-level session cache). `ExternalPlayscriptThread` + `ExternalPlayscriptEntry` render playscript format (speaker line, dialogue, non-adjacent parent arrows, 10-entry pagination). Thread button on `ExternalVesselCard` via synthetic `ReplyTarget`; suppressed for RSS and nostr_external
- Cross-platform like/favourite (Phase 4A, migration 092): `POST /api/v1/external-items/:id/like` validates linked-account ownership + protocol match, enqueues via `enqueueLike` (atproto/activitypub) or signs kind 7 + `enqueueNostrOutbound` (nostr_external). `enqueueLike` uses synthetic `nostr_event_id` = `like:{itemId}` for dedup. Feed-ingest dispatcher: `likeBlueskyRecord` (`app.bsky.feed.like` createRecord), `favouriteMastodonStatus` (resolve remote status + `POST /favourite`), Nostr kind 7 via existing signed-event relay publish. Frontend: interactive heart on `ExternalVesselCard` `EngagementRow` gated by `useLinkedAccounts` — three states: hidden (RSS), greyed/disabled (no linked account), interactive (optimistic +1, filled heart, error rollback). Migration 092 also adds `notes.external_parent_id` (Phase 4C prereq)
- Cross-platform repost/boost (Phase 4B): `POST /api/v1/external-items/:id/repost` validates linked-account ownership + protocol match, rejects nostr_external + rss (per §5.5 matrix). `enqueueRepost` uses synthetic `nostr_event_id` = `repost:{itemId}` for dedup. Feed-ingest dispatcher: `repostBlueskyRecord` (`app.bsky.feed.repost` createRecord), `reblogMastodonStatus` (resolve remote status + `POST /reblog`). Frontend: interactive repost-arrows button on `EngagementRow` — hidden for nostr_external/rss, greyed/disabled when no linked account, optimistic +1, crimson on repost
- Cross-platform inline reply + dual-write (Phase 4C): `POST /api/v1/external-items/:id/reply` validates linked-account ownership + protocol match, rejects rss. Signs kind 1 Nostr event via key-custody (NIP-10 tags for nostr_external), creates note with `external_parent_id` + feed_items dual-write, enqueues relay publish + outbound cross-post (`enqueueCrossPost` for atproto/activitypub, `enqueueNostrOutbound` for nostr_external). `InlineReplyBox` component: inline text field below cards and playscript thread entries, platform badge (`REPLYING VIA BLUESKY` etc.), linked-account gating (hidden for RSS, greyed/prompt when no account), optimistic reply count. `EngagementRow` reply button interactive. `ExternalPlayscriptEntry` Reply tag wired per-entry. `NoteVesselCard` renders `ParentContextTile` when note has `externalParentId`. Feed queries pipe `n.external_parent_id` to frontend
- Content warnings (Workspace Phase 5A, migration 093): `content_warning TEXT` on `external_items`. AP adapter captures `spoiler_text` from Mastodon's `sensitive + summary` fields. `ContentWarning` component wraps content with `.label-ui` spoiler text and `SHOW CONTENT` / `HIDE CONTENT` toggle. Session-only reveal state
- Poll display + voting (Workspace Phase 5B): AP adapter extracts `oneOf`/`anyOf` poll data into `interaction_data.poll`. `PollDisplay` component renders option bars with percentages, interactive voting for linked Mastodon accounts. `POST /api/v1/external-items/:id/poll-vote` enqueues via `enqueuePollVote`; feed-ingest `voteMastodonPoll` resolves remote status, fetches poll ID, POSTs to Mastodon `/api/v1/polls/:id/votes`
- Reader pane (Workspace Phase 5C): `GET /api/v1/extract?url=` endpoint uses `@mozilla/readability` + `jsdom` with SSRF-hardened `safeFetch` and 1h in-memory cache. `useReader` Zustand store + `ReaderPane` overlay (scrim + 640px serif panel, Escape/scrim to close). RSS article clicks open reader pane; fallback opens URL in new tab on extraction failure
- Inline video embeds (Workspace Phase 5D): `MediaBlock` detects YouTube/Vimeo URLs via regex, fetches oEmbed HTML from existing `/media/oembed` proxy on expand, renders iframe inline. Respects `prefers-reduced-motion`
- Pull-to-refresh + empty states (Workspace Phase 5E): `PullToRefresh` component (touch + wheel overscroll, 60px threshold, dampened pull indicator; desktop wheel accumulates upward delta while at scrollTop 0, 400ms decay; accepts explicit `scrollRef` from `Vessel` so default-height vessels work without relying on `findScrollParent` DOM traversal). `EmptyFeedTile` with `no-sources` / `no-items` / `caught-up` variants. `caught-up` renders above existing items when a pull-to-refresh returns no new content; both ADD SOURCES and DISMISS buttons dismiss the tile (parent-owned state), and scrolling up also dismisses it (`Vessel.tsx` uses a scroll listener for sized vessels + a wheel listener for default-height vessels where `scrollBodyRef` has no overflow scrolling). Tile only reappears after the next pull-to-refresh that finds no new content
- Context-only GC (Workspace Phase 5F): `external_context_gc` cron (daily 02:30 UTC) deletes `is_context_only` items older than 30 days with no references from `feed_items` or `notes.external_parent_id`
...
- `docs/adr/CARD-BEHAVIOUR-ADR.md` — unified card interaction model (click regions, conversational neighbourhood expansion, biddability tiers, author affordances). Phases 1–5 complete. External card bylines route to the internal **source surface** (`/source/[id]` page → `GET /api/v1/sources/:id` in `gateway/src/routes/sources.ts`), not the origin platform — the origin link lives on the source-attribution line. Neighbourhood hydration (`/external-items/:id/thread` + `/parent` + `/quote`) is rate-limited (30/min), timeout-capped (8s for the essential parent/quote/thread fetches, 2.5s for the optional grandparent-tag fetch), and returns a server-signalled `partial` flag. Rich embeds (addendum 2026-05-30): quoted posts render as nested `QuotedPostTile` mini-cards and external links render as `MediaBlock` link-preview cards, both in the workspace idiom (Bluesky + Mastodon; Nostr/RSS deferred). Phase 5 (2026-05-30) brought the **workspace** vessel cards (`web/src/components/workspace/VesselCard.tsx`) in line with this model — body-click expands the neighbourhood, the source-attribution line is the one route out (the `Open original` button is gone), and the byline links to the source surface; the `/feed/` cards (`ExternalCard`/`NoteCard`/`ArticleCard`) are being retired in favour of the workspace surface. Only the §VI.3 constructed external author profile remains deferred (its own ADR)
