...
- External content (RSS/Atom/JSON Feed, external Nostr, Bluesky, Mastodon) is ingested by `feed-ingest/` (Graphile Worker service + a long-lived Jetstream WebSocket listener, no HTTP port). The RSS adapter (`feed-ingest/src/adapters/rss.ts`) auto-detects JSON Feed responses (`application/feed+json` content-type or shape-sniff) and parses them natively, falling through to `rss-parser` for RSS/Atom
