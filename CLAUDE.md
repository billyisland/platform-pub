...
**Retiring a route into an overlay** (the direction of travel — everything that isn't the workspace or an overlay thereof is being retired): keep the old path as a thin server-component **redirect shim** to `/workspace?overlay=<name>[&…seed params]` (deep links, notification/email hrefs, and bookmarks must keep resolving), and have `WorkspaceView` read `window.location.search` once on mount (not `useSearchParams` — that would force a Suspense boundary on the prerendered `/workspace`) to `open()` the matching overlay store with the seeded state, then strip the params via `replaceState`. In-app links/`router.push` targets should point straight at `/workspace?overlay=<name>` to skip the redirect bounce. The dashboard (`web/src/app/dashboard/page.tsx` shim + `useDashboardOverlay`) is the reference implementation.
