...
- Pull-to-refresh + empty states (Workspace Phase 5E): `PullToRefresh` component (touch overscroll, 60px threshold, dampened pull indicator). `EmptyFeedTile` with `no-sources` / `no-items` variants, dismissible, session-only
...
- `docs/adr/WORKSPACE-FULL-VIEW-SPEC.md` — workspace full view spec (Compact/Full fidelity modes, engagement counts, parent context, threads, cross-platform interactions, reader pane, content warnings, polls, inline video, pull-to-refresh; all phases shipped)
