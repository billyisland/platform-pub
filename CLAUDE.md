...
- Pull-to-refresh + empty states (Workspace Phase 5E): `PullToRefresh` component (touch + wheel overscroll, 60px threshold, dampened pull indicator; desktop wheel accumulates upward delta while at scrollTop 0, 400ms decay). `EmptyFeedTile` with `no-sources` / `no-items` / `caught-up` variants, dismissible, session-only. `caught-up` renders above existing items when a refresh returns no new content, prompting the user to follow more accounts or add sources
