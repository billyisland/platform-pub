...
- `docs/adr/CARD-BEHAVIOUR-ADR.md` — unified card interaction model (click regions, conversational neighbourhood expansion, biddability tiers, author affordances, rich embeds). Phases 1–5 complete. Largely superseded by UNIVERSAL-POST-ADR for node identity + thread rendering (external bylines now route to `/author/:id`, not `/source/[id]`)
...
- `docs/adr/UNIVERSAL-POST-ADR.md` — unified **Post** model, feed assembly/ordering, single thread engine, and full-view rendering matrix. Supersedes the node-identity + thread-rendering portions of `UNIVERSAL-FEED-ADR.md` and `CARD-BEHAVIOUR-ADR.md`, the chronological-only following feed, and the HN-gravity score. **All phases shipped (0a–0c, 1, 2, 3, R, 4, 5 — the cutover).** The unified `PostCard` (`web/src/components/post/`) — fed by `GET /feed/:feedId` (`gateway/src/routes/post-feed.ts`, live §5 hotness scoring + dedup-to-one by `post_id`) and the `GET /thread/:postId` projector (`gateway/src/routes/post-thread.ts` + shared mapper `gateway/src/lib/post-mapper.ts`) — is the workspace's only card path; the dev flag and flag-off legacy (`VesselCard`, `useConversation`/`useExternalThread`/`useNeighbourhood`, `ConversationView`, etc.) are deleted. The reader pane (Phase R) is the `useReader` store + `ReaderOverlay` with URL-sync (native `/article/<dTag>`, external `/reader/<postId>`); external HTML is sanitised server-side via `shared/src/lib/sanitize.ts::sanitizeArticleContent`. Byline routing: tier-A/B external → `/author/:id` (`routes/author.ts`); tier-C/D plain text. The end-to-end id-bridges (`postId`, `externalItemId`, `external_author_id` — each must be threaded from `feeds.ts` `FEED_SELECT` through to the client adapter or thread-expand breaks) and the per-phase + content-pull bug-hunt history live in the ADR. **Scoped to the workspace** — the standalone `/feed` + `/source/[id]` + the `components/feed/` card family (and `GET /external-items/:id/thread`, the orphaned `GET /conversation/:eventId`) were left intact for a later `/feed`+`/source`-scoped pass.
