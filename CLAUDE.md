...
- **Network presences (Nostr root + satellites)**: the canonical identity is the custodial **Nostr root** minted at signup (`accounts.nostr_pubkey`/`nostr_privkey_enc`) — never replaced or preceded by another network. Every *other* network identity is a `network_presences` row (keyed `(account_id, protocol)`, one per network in v1), tagged `provenance` = **linked** (the user owns it; OAuth grant in `atproto_oauth_sessions` / `credentials_enc`) or **concierge** (all.haus minted it; secrets in key-custody). Concierge is **lazy** (minted only on explicit per-network opt-in, never at signup) and **native, not external** (a facet of `accounts`, never an `external_authors` tier-A twin — its posts must not re-ingest). Outbound dispatch targets only `lifecycle_state='active' AND is_valid`. Custodial identities are **export-mandatory**: `/account/export` ships the Nostr nsec via key-custody's gated `POST /keypairs/export`. Spec: `docs/adr/NETWORK-CONCIERGE-ADR.md` (Phase 0 + 1 shipped; atproto/AP concierge gated by §8.1).
