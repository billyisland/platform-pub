...
- `docs/adr/CARD-BEHAVIOUR-ADR.md` — unified card interaction model (click regions, conversational neighbourhood expansion, biddability tiers, author affordances). Phases 1–5 complete. External card bylines route to the internal **source surface** (`/source/[id]` page → `GET /api/v1/sources/:id` in `gateway/src/routes/sources.ts`), not the origin platform — the origin link lives on the source-attribution line. Neighbourhood hydration (`/external-items/:id/thread` + `/parent` + `/quote`) is rate-limited (30/min), 5s-timeout-capped, and returns a server-signalled `partial` flag. Rich embeds (addendum 2026-05-30): quoted posts render as nested `QuotedPostTile` mini-cards and external links render as `MediaBlock` link-preview cards, both in the workspace idiom (Bluesky + Mastodon; Nostr/RSS deferred). Phase 5 (2026-05-30) brought the **workspace** vessel cards (`web/src/components/workspace/VesselCard.tsx`) in line with this model — body-click expands the neighbourhood, the source-attribution line is the one route out (the `Open original` button is gone), and the byline links to the source surface; the `/feed/` cards (`ExternalCard`/`NoteCard`/`ArticleCard`) are being retired in favour of the workspace surface. Only the §VI.3 constructed external author profile remains deferred (its own ADR)
