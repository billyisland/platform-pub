...
- `ExternalCard` component renders external items with provenance badge (`VIA RSS` / `VIA NOSTR` / `VIA BLUESKY` / `VIA MASTODON`), sanitised HTML, images, link embed cards, quote-post links, video embeds (YouTube videos render as inline privacy-enhanced iframes; other videos show a "Watch on source" link). Atproto `at://` URIs are rewritten to `bsky.app` URLs at render time
