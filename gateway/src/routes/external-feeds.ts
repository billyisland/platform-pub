...
function validateSubscribeInput(input: {
  ...
  if (protocol === "rss") {
    try {
      ...
      if (u.protocol !== "http:" && u.protocol !== "https:")
        return "rss sourceUri must be http(s)";
    } catch {
      return "rss sourceUri must be a valid URL";
    }
    ...
    try {
      ...
      return "activitypub sourceUri must be a valid URL";
    }
  ...
  if (
    displayName != null &&
    (typeof displayName !== "string" || displayName.length > 200)
  ) {
  ...
  if (avatarUrl != null && avatarUrl.length > 0) {
    if (typeof avatarUrl !== "string" || avatarUrl.length > 2048)
      return "avatarUrl exceeds 2048 chars";
    try {
      const u = new URL(avatarUrl);
      if (u.protocol !== "https:") return "avatarUrl must be https";
    } catch {
      return "avatarUrl must be a valid URL";
    }
  }
    ...
    for (const r of relayUrls) {
      ...
      try {
        ...
        if (u.protocol !== "ws:" && u.protocol !== "wss:")
          return "relayUrls must use ws:// or wss://";
      } catch {
        return `invalid relayUrl: ${r}`;
      }
    }
  }
    ...
    async (req, reply) => {
      ...
      try {
        const result = await withTransaction(async (client) => {
          ...
          await client.query(
            `SELECT pg_advisory_xact_lock(hashtext('feed_sub:' || $1::text))`,
            [readerId],
          );
          const {
            rows: [{ count: subCount }],
        ...
        if ((err as { code?: string } | null)?.code === "SUB_LIMIT_EXCEEDED") {
          return reply
            .status(429)
            .send({ error: `Subscription limit reached (max ${maxSubs})` });
        }
