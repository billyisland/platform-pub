...
> **Status, 2026-07-22 — superseded-in-waiting by `docs/adr/WORKSPACE-COLUMN-LAYOUT-ADR.md`.** That ADR keeps the *goal* of this addendum (a drop never disturbs anything at a distance) and discards the *mechanism*: with geometry derived from a stored column/slot order, an overlapping resting state is unrepresentable, so `findRestingPosition`, `clampSizeClear`, the bootstrap heal and `settleAfterAbandonedMerge` are deleted rather than tuned — the declined-merge relocation this addendum's solver still causes has no successor because the source's slot is held open for the whole gesture. **This addendum still governs until that ADR's Slice 3 lands** (Slice 0, virtualization, shipped 2026-07-22; Slices 1–5 open) — defend it as written until then, but build no new mechanism on it.
