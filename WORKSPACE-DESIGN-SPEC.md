...
A floor as tall as the user's screen and as wide as they need. Vertically it is a single rectangle — the screen, at whatever size, never taller. Horizontally it extends without limit: dragging a vessel past the current edge stretches the space, scrolling sideways slides over it, and moving the vessel back contracts it again. At rest the floor is *exactly* the span of the vessels plus breathing room — the room to grow opens only while a drag is live, so "without limit" means the space yields when asked, never that empty floor is pre-stretched waiting. Ctrl+←/→ jumps the view to the floor's far ends (suppressed inside text fields and while an overlay is open). Extent is *derived* from where the vessels sit, never stored, so there is no stale empty region to reclaim and nothing to reset. The asymmetry is deliberate — vertical space is the reading dimension and stays bounded by the screen so a feed never hides below the fold; lateral space is the arranging dimension and is the user's to take. The workspace floor is grey-100. Feeds are heavy black ⊔ shapes arranged on the floor, with space between them. No hairlines, no keylines, no drawn frames: the walls of the vessels are the only visual framing in the system, and space does the rest. The grammar leaves no place where a hairline would fit.
...
*Position.* Drag to move. The vessel goes where you put it. Other vessels reflow to accommodate the new position. No auto-arranging. Horizontal movement is unbounded (it stretches the floor); vertical movement is clamped to the screen. Movement snaps to an unobtrusive lattice (`GRID` in `web/src/lib/workspace/grid.ts`) — invisible at the gesture level, it just keeps edges tidy. It was 20px, chosen as LCM(10, 4) so the floor stayed in phase with the 4px design rhythm (Tailwind spacing + the 4px slab) and content shared a lattice with the chrome; **halved to 10px on 2026-07-20** for finer placement, which gives up that phase relationship but keeps the lattice even, so vessels and panes still land on whole pixels. 8px is the nearest value that would halve it *and* keep the 4px phase, if the drift ever shows. The same lattice governs Glasshouse pane drag/resize. Position persists.
...
**The wave is best-effort; the guarantee is verify-and-repair (2026-07-21).** The push-propagation wave produces the *good* resolution — displacement proportional to the gesture — but it does not always converge: wide movers (a resize commit is a mover) can livelock it on legal geometry, at any propagation budget. So the resolver's contract has two layers: the wave for quality, then a mandatory self-check whose fallback deterministically **shelves** any still-overlapping vessel past the right edge of everything settled — the escape valve applied wholesale. A shelved vessel jumps rather than shuffles; that is the accepted cost of making the invariant a guarantee instead of an intent. The same repair primitive heals persisted layouts on hydrate, because a resting pile renders as one vessel and the user can neither see nor fix what the mover-scoped resolver will never revisit.
...
## Addendum — Mover-yields placement: nothing else on the floor ever moves (2026-07-21)

> **Status, 2026-07-22 — SUPERSEDED by `docs/adr/WORKSPACE-COLUMN-LAYOUT-ADR.md` (its Slice 3 shipped the same day).** That ADR keeps the *goal* of this addendum (a drop never disturbs anything at a distance) and discards the *mechanism*: with geometry derived from a stored column/slot order, an overlapping resting state is unrepresentable, so `findRestingPosition`, `clampSizeClear`, the bootstrap heal and `settleAfterAbandonedMerge` are **deleted** — `web/src/lib/workspace/collision.ts` and `canvas.ts` no longer exist. The declined-merge relocation this addendum's solver caused has no successor because the source's slot is held open for the whole gesture. **Everything below is history**: read it for the reasoning that got us here (the resting-state reading, the merge-as-pointer-question split, and the horizontal escape valve all carried forward in changed form), not as a rule to defend.

**What changed and why.** The 2026-07-20 addendum's *mechanism* — push-what-you-hit, propagated in waves, backstopped by verify-and-repair — is superseded. Enforcing no-overlap by displacing *other* vessels made the floor feel glitchy: putting one feed down could bounce a neighbour (or a chain of them) aside, and the livelock/shelving machinery existed only to make that displacement safe. Third-party displacement was the surprise, not the no-overlap rule. The invariant, the resting-state reading, the merge-as-pointer-question split, and the horizontal escape valve all stand unchanged; only *who yields* is reversed.

**The rule: the mover yields; obstacles are immovable.** A gesture moves exactly the vessel under the user's hand. On release, that vessel settles at the nearest clear, lattice-aligned, in-bounds position (`findRestingPosition`, `web/src/lib/workspace/collision.ts`) — obstacles never move, so a drop can never surprise the user at a distance. Mid-drag there is no placement work at all: the held vessel rides over the floor (transient raised z-order), because the invariant governs the resting state, not the gesture. Horizontal remains the escape valve — at the mover's own y there is always clear ground past the furthest right edge, which is what makes placement always succeed.

**Resize clamps instead of settling.** A stretch must not displace anything either — including the resized vessel itself, whose anchored edge the user placed deliberately. So resize keeps the invariant from the other side: each proposed size is clamped at the first neighbour it would hit (`clampSizeClear`, applied per frame via the Vessel's `clampResize` prop), and the handle visibly stops there. The commit inherits the clamp, so a resize needs no resolution pass.

**Merge composes the same way, minus the suspension.** Arming is unchanged (pointer inside the target). The per-vessel collision suspension is gone because there is nothing to suspend — nothing moves mid-drag anyway. On a declined or failed merge the *source* settles clear via the same mover-yields placement, which also reads correctly: the vessel the user dragged slides off; the target they declined to merge into stays put. (Under the push model the cancel path shoved the target away.)

**What survives of the old machinery.** `repairRestingLayout` (the deterministic shelving repair) survives with one caller: the store's bootstrap reconcile (`reconcileLayouts`), for layouts persisted by the pre-2026-07-21 push resolver, whose livelocked waves could rest vessels at identical coordinates. It runs at bootstrap rather than hydrate — once the server feed list is known — because the heal must be **visibility-aware**: a vessel resting over a *hidden* feed's stored rect is invariant-conforming (hidden feeds are not obstacles), and a hydrate-time heal, blind to hidden and deleted-elsewhere feeds, shelved legal arrangements. The reconcile also prunes ghost layouts for feeds deleted on another device. The wave, its propagation budget, and the in-resolver verify-and-repair pass are deleted — mover-yields placement is correct by construction (a single vessel is placed against a fixed set of obstacles), so there is no intermediate state to verify.

**System placements yield too (2026-07-21).** The mover-yields rule is universal, not gesture-scoped: `adoptFeed`, the bootstrap default slots, and un-hide each settle their requested position through `findRestingPosition` before it lands (bootstrap against store-estimate obstacles, since the DOM isn't up yet; a restored feed yields to whatever legally occupied its ground while it was hidden). A system write path that places without settling is how the no-overlap guarantee regresses.
