...
**What survives of the old machinery.** `repairRestingLayout` (the deterministic shelving repair) survives with one caller: the store's hydrate heal, for layouts persisted by the pre-2026-07-21 push resolver, whose livelocked waves could rest vessels at identical coordinates. The wave, its propagation budget, and the in-resolver verify-and-repair pass are deleted — mover-yields placement is correct by construction (a single vessel is placed against a fixed set of obstacles), so there is no intermediate state to verify.
