...
| `RESONANCE_GLYPH_ENABLED`                   | gateway                                | Master switch for the resonance glyph on feed cards (SOCIAL-PROOF-RESONANCE-ADR D7 / step 4). `0`/unset = ships dark: the gateway **mapper** nulls `Post.resonanceBand` on every read path, so no band leaves the server and no glyph can render; `1` = enabled. The scoring crons write `feed_items.resonance*` regardless — this gates display only, so flipping it needs no backfill and is instantly reversible. Do **not** flip until band-3 incidence has been re-measured per protocol on prod volume (the migration-160 gates are dev-tuned; CONSOLIDATED-TODO §9.12). No web twin — deliberately one home |
