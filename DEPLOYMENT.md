...
| `MASTODON_ASSISTED_ENABLED`                 | gateway                                | Master switch for ASSISTED activitypub ("set one up for me" → Mastodon via signup-then-resume OAuth round-trip). `0`/unset = ships dark (`POST /linked-accounts/mastodon/assisted` 503s, UI keeps "Set one up · soon"); `1` = enabled. Built dark 2026-06-11; flip only after one real signup round-trip is verified. See `docs/adr/NETWORK-CONCIERGE-ADR.md` §9 |
