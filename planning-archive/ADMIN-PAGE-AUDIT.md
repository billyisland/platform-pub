...
**Batch 3 (Page shell unification):** Follow-up after visual inspection revealed that even after Batch 2, page titles sat at different heights and left positions across pages. Root cause: no shared page-frame primitive — each page hand-rolled `mx-auto max-w-* px-4 sm:px-6 py-*` with inconsistent values (`py-8` / `py-10` / `py-12`, title `mb-6` / `mb-8` / `mb-10`, Network's one-off `pt-16 lg:pt-0` mobile shim). Introduced `<PageShell>` and `<PageHeader>` in `web/src/components/ui/PageShell.tsx` fixing outer padding (`py-12`), title styling, and title→content gap (`mb-8`). Width is a single prop: `article` (640px, forms), `feed` (780px, lists/cards), `content` (960px, tables/dashboards). Migrated Settings, Profile, Network, Library, Ledger, Admin Reports, Dashboard, and Traffology layout. Admin Reports moved from `content` to `feed` (card list, not a table). Rule codified in CLAUDE.md.
