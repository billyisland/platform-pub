...
> **ARCHIVED 2026-07-14.** Part 1 (all of §1.1–§1.8) shipped in full; §1.7 decided (no durable-execution engine); §2.2 decided (Dial A) and shipped. The remaining open items are all gated — the §2.1 Stripe posture fork + §1.6 merchant-posture module wait on the §2.3 counsel sign-off (baseline residual #1), and the §1.3(2) pledge / §1.3(4) traffology tests ride with their mothballed features. Those live in `docs/audits/CONSOLIDATED-TODO.md` (§1 item 12 · §2 item 5 · §10). This doc is retained as the detailed record (verified file:line targets, the fork analysis); shipped-work records are in `docs/audits/FIX-PROGRAMME.md`.
