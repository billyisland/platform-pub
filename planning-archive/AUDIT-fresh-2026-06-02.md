...
> - **#1 — FIXED.** Investigation refined the finding: `schema.sql` already carried 098/099 DDL (hand-appended out of pg_dump section order) but was **missing migration 100** (`repost_edges`) and, more importantly, seeded **no `_migrations` rows** — so a fresh boot left `_migrations` empty and the next `migrate.ts` run re-ran `001`+ against the already-built schema. (Migrations are incremental on top of `schema.sql`, so "make migrate the only path" isn't viable — `schema.sql`-as-base is the mechanism; the fix is to keep the two halves consistent.) Regenerated `schema.sql` from a fully-migrated DB (canonicalizing the 098/099 blocks, adding 100, preserving `graphile_worker`/`traffology`) and appended a documented `_migrations` seed of all 101 filenames. Verified end-to-end: fresh DB from the new `schema.sql` → real `migrate.ts` runner reports "All migrations already applied." DEPLOYMENT.md corrected (097→101, table count 88→73, migrations table extended to 101, the now-true seed behaviour documented).
> - **#4 — CORRECTION: largely a false finding.** The resolver is *not* stubbed. `classifyInput` classifies `bluesky_handle`/`fediverse_handle`/`did`/`ambiguous_at`, and the dispatch runs real Phase-B chains backed by fully-implemented `resolveAtproto`/`resolveActivityPubHandle`/`resolveActivityPubByActor`. The stale header comment at `resolver.ts` has since been replaced with an accurate description of the real Phase-B chains — no action remaining. (See also the related overstatement in #10: `web/` is not unlinted — it runs `next lint`; the accurate point is only that the *root* config's floating-promise rule doesn't reach `web/`.)
> - **#8 — FIXED.** All three dead exports removed. `shared/src/auth/keypairs.ts` deleted outright (its sole export `getAccountPubkey` had zero callers; `shared` uses subpath exports, so there was no barrel re-export to clean — the `dist/index.d.ts` re-export is gitignored build output). `validateWebSocketUrl` (the back-compat shim) removed from `shared/src/lib/http-client.ts`. `TabSettlement` removed from `payment-service/src/types/index.ts` — which also orphaned `WriterPayout`/`HandledStripeEvent` (the #9 unused-locals in the same file) and, transitively, `PayoutStatus`; all four removed together.
> - **#9 — FIXED.** All 24 `no-unused-vars` eslint warnings cleared across the backend (eslint `no-unused-vars` count 24 → 0; total warnings 143 → 118, the remainder being the #7 `no-explicit-any` set). Unused imports/locals removed; the one unused *arg* (`trust-epoch-aggregate.ts` `helpers`) prefixed `_helpers`. The four assigned-but-never-used locals were each confirmed side-effect-free before removal: `moderation.ts` `reason`, `my-account.ts` `account`, `scheduler.ts` `INTERNAL_SERVICE_TOKEN` (dead — the real fetch uses `INTERNAL_SECRET`), and `accrual.ts` `config` (`getConfig()` is idempotent cache-priming; accrual tests still pass). Verified: `shared` builds, all five touched services `tsc --noEmit` clean, payment-service (41) + touched feed-ingest (22) tests pass.
> - **#10 — FIXED (promise-safety scope).** The root flat config no longer ignores `web/**`: `eslint.config.mjs` now applies the same type-aware promise-safety rules (`no-floating-promises`/`no-misused-promises`/`await-thenable`) + hygiene set to `web/src/**/*.{ts,tsx}` as the backend, so `npm run lint` finally reaches the 254-file frontend. JSX handlers needed `checksVoidReturn.attributes: false` (React ignores a handler's returned promise — that one option had silenced 153 false positives). The first real run surfaced **97 errors**: 80 `no-floating-promises`, 14 `no-duplicate-imports`, 2 `restrict-plus-operands`, 1 `await-thenable` — all fixed (fire-and-forget → `void`; meaningful save/submit/vote paths → `.catch(console.error)`; merged duplicate imports; coerced `+` operands). Two were latent bugs, not noise: `ArticleReader` reset its loading flag before an un-awaited unlock finished (added `await`), and `ArticleEditor`'s `onPublish` prop was typed `=> void` while its caller is `async` (widened to `=> void | Promise<void>`). React/hooks/a11y/next-image rules stay the domain of `web`'s own `next lint` (a separate, currently-dormant pass left for later, per the audit's "promise-safety is the accurate gap" framing in #4); their inline `eslint-disable` directives are kept valid here via dependency-free no-op rule stubs (eslint@10 outruns those plugins' peer ranges, so installing them just to validate comments was the wrong trade). Verified: full-repo `npm run lint` 0 errors (227 warnings, all pre-existing hygiene debt), web `tsc --noEmit` clean, web vitest 136/136 pass, no new hairlines.
